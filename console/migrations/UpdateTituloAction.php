...
class UpdateTituloAction extends Action
{
    public function run()//$filial_id)
    {
        ...
        while (($line = fgetcsv($file,null,';')) !== false)
        {
            ...
            $produto_busca = Produto::find()->andWhere(['=','codigo_fabricante', $line[0].".M"])->one();
            if($produto_busca){
                echo " - produto encontrado";
                $produtosArray[$produto_busca->id] = $produto_busca->id;
            }
            else{
        ...
        foreach ($filials as $filial) {
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
                ...
                $produto_filiais = $filial  ->getProdutoFilials()
                                            ...
                                            ->andWhere(['produto_filial.produto_id' => $produtosArray])
