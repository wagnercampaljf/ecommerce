...
class TestePedroAction extends Action
{
    public function run($q)
    {
        // $produtos = Produto::findAll(13929);

        // foreach ($produtos as $produto) {

        //     $nota_fiscal_produto = NotaFiscalProduto::find()
        //         ->join('join', 'nota_fiscal', 'nota_fiscal.id = nota_fiscal_produto.nota_fiscal_id')
        //         ->where("nota_fiscal_produto.pa_produto = 'PA'||$produto->id and nota_fiscal.tipo_nf = 0 and nota_fiscal.finalidade_emissao <> 4")->orderBy(['nota_fiscal.data_nf' => SORT_DESC])->one();
        //     if ($nota_fiscal_produto) {

        //         $imposto = ($nota_fiscal_produto->valor_icms + $nota_fiscal_produto->valor_ipi + $nota_fiscal_produto->valor_seguro + $nota_fiscal_produto->valor_total_frete + $nota_fiscal_produto->outras_despesas) - $nota_fiscal_produto->valor_desconto;
        //         $imposto = $imposto / $nota_fiscal_produto->qtd_comercial;
        //         $nota_fiscal_produto->valor_real_produto = ($nota_fiscal_produto->valor_unitario_tributacao * 100) / 70 + $imposto;
        //         $nota_fiscal_produto->save(false);

        //         echo $produto->nome . ' - ' . $produto->codigo_global . ' - ' . $nota_fiscal_produto->valor_real_produto . "\n";

        //         $produto_filial = ProdutoFilial::find()->where("produto_id = $produto->id and filial_id = 96")->one();

        //         $modelValorProdutoFilial = new ValorProdutoFilial();
        //         $modelValorProdutoFilial->valor = $nota_fiscal_produto->valor_real_produto * 1.82;
        ...
        //         $modelValorProdutoFilial->valor_compra = $nota_fiscal_produto->valor_real_produto;
        //         $modelValorProdutoFilial->save(false);
        ...
        echo "INÍCIO TESTE\n\n";
        echo '<pre>';
        print_r(PedidoMercadoLivre::baixarPedidoML($q, false));
        echo '</pre>';
        die;
