...
class TestePedroOkAction extends Action
{
	...
	echo "INÍCIO TESTE\n\n";
        ...
        $filial = '';
        $nota_fiscal = NotaFiscal::findOne(13712)->cod_empresa;
        ...
        $nota_fiscal_produto = NotaFiscalProduto::findOne(['nota_fiscal_id' => 13712]);
        ...
        foreach ($nota_fiscal_produto as $produto) {
            ...
            if ($produto->pa_produto !== '') {
                ...
                $valor_unitario = $produto->valor_produto / $produto->qtd_comercial;

                $modelValorProdutoFilial = new ValorProdutoFilial();
                $modelValorProdutoFilial->valor = $valor_unitario * 1.73;
                $modelValorProdutoFilial->dt_inicio = date("Y-m-d H:i:s");
                $modelValorProdutoFilial->produto_filial_id = $filial;
                $modelValorProdutoFilial->promocao = false;
                $modelValorProdutoFilial->valor_compra = $valor_unitario;
...
Skype: pecaagora";
	...
	var_dump(\Yii::$app->mailer   ->compose()
        ...
        $produtos_filiais = ProdutoFilial::find()->andWhere(["=", "filial_id", 96])
