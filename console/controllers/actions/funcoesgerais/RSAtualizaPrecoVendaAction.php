...
class RSAtualizaPrecoVendaAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_pecaagora_RS-15-03-2021.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_produtos_pecaagora_RS-15-03-2021.csv")){
            unlink("/var/tmp/log_produtos_pecaagora_RS-15-03-2021.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_pecaagora_RS-15-03-2021.csv", "a");
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as informações de preços a subir
            ...
            $preco_compra   = (float) str_replace(",",".",str_replace(".","",$linhaArray[6]));
            ...
            if ($produto){ //Se encontrar o produto, processa o preço
                ...
                $produto_filial = ProdutoFilial::find()->andWhere(['=','produto_id',$produto->id])->andWhere(['=','filial_id',82])->one();
