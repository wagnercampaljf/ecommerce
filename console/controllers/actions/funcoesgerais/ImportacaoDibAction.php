...
class ImportacaoDibAction extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        $file = fopen('/var/tmp/dib_produtos.csv', 'r');
        ...
        if (file_exists("/var/tmp/log_importar_dib.csv")){
            unlink("/var/tmp/log_importar_dib.csv");
        }
        
        $arquivo_log = fopen("/var/tmp/log_importar_dib.csv", "a");
        // Escreve no log
        ...
        foreach ($LinhasArray as $k => &$linhaArray ){
            ...
            $produto = new Produto();
            $produto->codigo_fabricante = "D".$linhaArray[0];
            ...
            $produto->fabricante_id     = 123;
            $this->slugify($produto);
            //print_r($produto);
            if ($produto->save()){
                echo "Produto CRIADO\n";
                fwrite($arquivo_log, $linhaArray[0].";".$linhaArray[1].";".$linhaArray[3].";Produto CRIADO");
            } else{
                fwrite($arquivo_log, $linhaArray[0].";".$linhaArray[1].";".$linhaArray[3].";Produto NAO CRIADO\n");
            ...
            $preco = (float)str_replace(" ","",(str_replace("@",".",(str_replace(".",",",str_replace(",","@",$linhaArray[3]))))));
