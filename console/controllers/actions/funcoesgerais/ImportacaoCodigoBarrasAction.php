...
class ImportacaoCodigoBarrasAction extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        $file = fopen('/var/tmp/codigo_barras_lng.csv', 'r');
        ...
        if (file_exists("/var/tmp/log_importacao_codigo_barras.csv")){
            unlink("/var/tmp/log_importacao_codigo_barras.csv");
        }
        
        $arquivo_log = fopen("/var/tmp/log_importacao_codigo_barras.csv", "a");
        // Escreve no log
        ...
        foreach ($LinhasArray as $k => &$linhaArray){
            ...
            if ($codigo_fabricante == null or $codigo_fabricante == ""){
                ...
                $produto_filial = ProdutoFilial::find()->joinWith(['produto'])
                                                ...
                                                ->where("filial_id = 60 and (produto.codigo_fabricante = '".$codigo_fabricante."' or produto.codigo_fabricante = '".$codigo_fabricante_l."')")
                                                ->one();
                ...
                if (isset($produto_filial)){
                    ...
                    $produto->codigo_barras = $linhaArray[2];
                    $produto->save();
