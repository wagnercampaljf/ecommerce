...
class GaussAtualizaPrecoVendaAction extends Action
{
    public function run(){
        ...
        //$file = fopen("/var/tmp/produtos_gauss_2021-06-23_precificado.csv", 'r');
        $file = fopen("/var/tmp/produtos_gauss_2021-06-25_precificado.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_produtos_gauss_2021-06-25_precificado.csv")){
            unlink("/var/tmp/log_produtos_gauss_2021-06-25_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_gauss_2021-06-25_precificado.csv", "a");
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as informações de preços a subir
            ...
            if ($produto){ //Se encontrar o produto, processa o preço
                ...
                $produto_filial = ProdutoFilial::find()->andWhere(['=','produto_id',$produto->id])->andWhere(['=','filial_id',94])->one();
