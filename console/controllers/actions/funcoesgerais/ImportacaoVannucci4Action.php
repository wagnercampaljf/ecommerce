...
class ImportacaoVannucci4Action extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        $file = fopen('/var/tmp/750_1115_pellegrino_vannucci_precificado.csv', 'r');
        ...
        if (file_exists("/var/tmp/log_importar_precos_vanucci_750-1115.csv")){
            unlink("/var/tmp/log_importar_precos_vanucci_750-1115.csv");
        }
        
        $arquivo_log = fopen("/var/tmp/log_importar_precos_vanucci_750-1115.csv", "a");
        // Escreve no log
        ...
        foreach ($LinhasArray as $k => &$linhaArray ){
            ...
            if ($codigo_global == null and $codigo_global == ""){
                ...
                $valorProdutoFilial->valor              = (float)str_replace(",",".",$linhaArray[11]);
                $valorProdutoFilial->valor_cnpj         = (float)str_replace(",",".",$linhaArray[11]);
