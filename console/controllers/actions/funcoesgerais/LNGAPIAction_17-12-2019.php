...
class LNGAPIAction extends Action
{
	...
	$arquivo_log = fopen("/var/tmp/log_lng_api".date("Y-m-d_H-i-s").".csv", "a");
        fwrite($arquivo_log, "esta rodando a rotina");
        fclose($arquivo_log);
        die;
        ...
        foreach ($produtos_codigo as $k => $codigo){
            ...
            if(!isset($produto_dados->errorCode)){
                ...
                if($produto_peca){
                    ...
                    $produto_novo->nome                     = str_replace("  ","",$produto_dados->nome);
                    ...
                    $produto_novo->codigo_similar           = str_replace("  ","",$produto_dados->codigo_original);
