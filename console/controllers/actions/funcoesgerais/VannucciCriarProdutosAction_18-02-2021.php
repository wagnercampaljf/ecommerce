...
class VannucciCriarProdutosAction extends Action
{
    public function run(){
        ...
        if (file_exists("/var/tmp/log_vannucci_ausentes_completo_27-04-2020_02.csv")){
            unlink("/var/tmp/log_vannucci_ausentes_completo_27-04-2020_02.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_vannucci_ausentes_completo_27-04-2020_02.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            echo "\n".$i." - ".$linhaArray[4]." - ".$linhaArray[1]." - ".$linhaArray[17]." - ".$linhaArray[18]." - ".$linhaArray[19]; //Exibe no console(Terminal) as informações dos preços durante o processamento
            ...
            if (!$produto){ //Se encontrar o produto, processa o preço
                ...
                $produto_novo->codigo_fabricante    = $linhaArray[4];
                $produto_novo->nome                 = $linhaArray[1];
                $produto_novo->peso                 = $linhaArray[12]/1000;
                $produto_novo->altura               = $linhaArray[13];
                $produto_novo->largura              = $linhaArray[14];
                $produto_novo->profundidade         = $linhaArray[15];
                $produto_novo->subcategoria_id      = $subcategoria->id;
                $produto_novo->aplicacao            = $linhaArray[5];
                $produto_novo->codigo_montadora     = $linhaArray[3];
                $produto_novo->codigo_barras        = $linhaArray[6];
                $produto_novo->descricao            = $linhaArray[5];
                ...
                $preco_compra = $linhaArray[18];
                $quantidade = $linhaArray[17];
