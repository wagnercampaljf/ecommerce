...
class BRCriarProdutosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_br_não_cadastrados.csv", 'r'); //Abre arquivo com preços para subir
        ...
        if (file_exists("/var/tmp/log_produtos_br_não_cadastrados.csv")){
            unlink("/var/tmp/log_produtos_br_não_cadastrados.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_br_não_cadastrados.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            fwrite($arquivo_log, "\n".'"'.$linhaArray[0].'";"'.$linhaArray[1].'";"'.$linhaArray[2].'";"'.$linhaArray[3].'";"'.$linhaArray[4].'";"'.$linhaArray[5].'";"'.$linhaArray[6].'";"'.$linhaArray[7].'";"'.$linhaArray[8].'";"'.$linhaArray[9].'";"'.$linhaArray[10].'";"'.$linhaArray[11].'";"'.$linhaArray[12].'";"'.$linhaArray[13].'";"'.$linhaArray[14].'";"'.$linhaArray[15].'";"'.$linhaArray[16].'";');
            ...
            if (!$produto){ //
                ...
                $produto_novo->codigo_fabricante    = $linhaArray[15];
                $produto_novo->nome                 = $linhaArray[2]." ".$linhaArray[3]." ".strtoupper ( $linhaArray[10] );
                $produto_novo->peso                 = 5;
                ...
                $produto_novo->descricao            = $linhaArray[2]." ".$linhaArray[3]." ".$linhaArray[10];
