...
class LNGAPIAction extends Action
{
	...
	$contador = 0;
        ...
        foreach ($produtos_codigo as $k => $codigo){
            ...
            //if(!(str_replace(" ","",$codigo) == "42-186")){ continue;}
	    ...
	    curl_setopt($ch, CURLOPT_URL, "http://189.112.136.145:86/restprod/PRODUTOS?codproduto=".$codigo_fabricante."&cuseraccount=OptLng&csenhaaccount=O!891104lnop:&cCNPJ=18947338000463");
		    ...
		    if(!is_null($produto_dados->codigo_original) && $produto_dados->codigo_original != ""){
			    ...
			    if($produto_filial->e_atualizar_quantidade_planilha){
	                            if($produto_filial->quantidade != $produto_dados->estoque){
                            ...
                            $valor_atual = 0;
                            $valor_produto_filial_atual = ValorProdutoFilial::find()->andWhere(["=", "produto_filial_id", $produto_filial->id])->orderBy(["id"=>SORT_DESC])->one();
                            if($valor_produto_filial_atual){
                                $valor_atual = $valor_produto_filial_atual->valor;
                            }
                            
                            if($valor_atual != $preco_venda){
