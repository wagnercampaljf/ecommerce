...
class DibCriarProdutosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_dib-sem_cadastro.csv", 'r'); //Abre arquivo com preços para subir
        ...
        if (file_exists("/var/tmp/log_produtos_dib-sem_cadastro.csv")){
            unlink("/var/tmp/log_produtos_dib-sem_cadastro.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_dib-sem_cadastro.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            echo "\n".$i." - ".$linhaArray[0]." - ".$linhaArray[1]." - ".$linhaArray[6]." - ".$linhaArray[8]; //Exibe no console(Terminal) as informações dos preços durante o processamento
            ...
            if (!$produto){ //
                ...
                $produto_novo->codigo_fabricante    = 'D'.$linhaArray[0];
               ...
               /* if($quantidade == 0){
                ...
                $produto_filial_novo->filial_id                     = 97;
