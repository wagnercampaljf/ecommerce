...
class ImportacaoBRAction extends Action
{
    public function run(){
        ...
        if (file_exists("/var/tmp/log_importar_BR.csv")){
            unlink("/var/tmp/log_importar_BR.csv");
        }

        $arquivo_log = fopen("/var/tmp/log_importar_BR.csv", "a");
        // Escreve no log
	    ...
	    $preco = str_replace(",",".",$linhaArray[18]);
