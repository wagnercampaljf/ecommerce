...
class PiracicabaAtualizaPrecoVendaAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_Piracicaba-24-11-2020.csv", 'r');
        ...
        $log = "/var/tmp/log_produtos_Piracicaba-24-11-2020.csv";
        if (file_exists($log)){
            unlink($log);
        }
            ...
            if ($produto){
               ...
               // echo  " - Produto encontrado - ".$produto->id." - MarcaID: ".$produto->marca_produto_id;
                ...
                $produto_filial = ProdutoFilial::find()->andWhere(['=','produto_id',$produto->id])
                    ->andWhere(['=','filial_id',62])
                    ->one();


                if($produto_filial){

                    $produto_filial->quantidade = (int) $linhaArray[6];
