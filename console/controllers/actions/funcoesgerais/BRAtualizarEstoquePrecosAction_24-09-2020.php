...
class BRAtualizarEstoquePrecosAction extends Action
{
    public function run(){
        ...
        $produtos_br_planilha = array();

        while (($line = fgetcsv($file,null,';')) !== false)
        {
            $LinhasArray[] = $line;
            $produtos_br_planilha[$line[14]] = $line;
        }
        fclose($file);
        ...
        foreach($produtos_filial_br as $k => $produto_filial_br){
            ...
            if(array_key_exists($produto_filial_br->produto->codigo_fabricante, $produtos_br_planilha)){
                $produto_encontrado = true;
            }

            if(!$produto_encontrado){
                echo "\n".$k." - ".$produto_filial_br->produto->codigo_fabricante." - produto não encontrado na planilha";

                fwrite($arquivo_log, "\n".'"'.$produto_filial_br->produto->id.'";"'.$produto_filial_br->produto->codigo_fabricante.'";"'.$produto_filial_br->quantidade.'";"Produto não encontrado na planilha"');
            }
        }
