...
class VannucciAtualizarPrecosAction extends Action
{
    public function run(){
       ...
       // $file = fopen("/var/tmp/vannuci_produtos_23-03-2021_precificado.csv", 'r');
        //$file = fopen("/var/tmp/vannuci_produtos_14-04-2021_precificado.csv", 'r');

        $file = fopen("/var/tmp/vanucci_precos_2021-05-19_precificado.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_vanucci_precos_2021-05-19_precificado.csv")){
            unlink("/var/tmp/log_vanucci_precos_2021-05-19_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_vanucci_precos_2021-05-19_precificado.csv", "a");
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            if ($i <= 1251){ //Pula a primeira linha de preços, por se tratar dos títulos da planilha
