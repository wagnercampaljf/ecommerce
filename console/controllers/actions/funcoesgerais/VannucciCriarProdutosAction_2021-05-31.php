...
class VannucciCriarProdutosAction extends Action
{
    public function run(){
        ...
        if (file_exists("/var/tmp/log_vannucci_novos_produtos-03-05-2021_precificado.csv")){
            unlink("/var/tmp/log_vannucci_novos_produtos-03-05-2021_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_vannucci_novos_produtos-03-05-2021_precificado.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            $produto = Produto::find()  ->andWhere(['=','codigo_fabricante', $linhaArray[7]])
            ...
            if (!$produto){ //Se encontrar o produto, processa o preço
                ...
                $produto_novo->codigo_fabricante    = $linhaArray[7];
