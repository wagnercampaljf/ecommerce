...
class CompararPlanilhasVannucciAction extends Action
{
    public function run(){
        ...
        $file = fopen('/var/tmp/planilha-nova_09-11-2020.csv', 'r');
        while (($line = fgetcsv($file,null,';')) !== false)
        {
            $LinhasArrayNova[] = $line;
        }
        fclose($file);
        if (file_exists("/var/tmp/log_comparar_planilha-nova_09-11-2020.csv")){
            unlink("/var/tmp/log_comparar_planilha-nova_09-11-2020.csv");
        }
        $arquivo_log_novo = fopen("/var/tmp/log_comparar_planilha-nova_09-11-2020.csv", "a");
        ...
        foreach ($LinhasArrayNova as $i => &$LinhaArrayNova ){
            ...
            if($produto_filial_sorocaba){
                echo " - Encontrado";
                fwrite($arquivo_log_novo, "encontrado;");
            }
            else{
                echo " - Não encontrado";
                fwrite($arquivo_log_novo, "Não encontrad0;");


            }

            $preco_venda->valor_cnpj = $preco_venda->valor_cnpj*1.14;
            $preco_venda->valor_cnpj = number_format($preco_venda->valor_cnpj, 2, '.', '');

            fwrite($arquivo_log_novo,  $LinhaArrayNova[0].";".$preco_venda->valor_cnpj.";".$quantidade ."\n");
        }
