...
class ReplicarImagemBRAction extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        //$file = fopen('/var/tmp/replicar_imagens_2021-05-17.csv', 'r');

        $file = fopen('/var/tmp/replicar_imagens_2021-05-31.csv', 'r');
       ...
       foreach ($LinhasArray as $k => &$linhaArray ){
           ...
           $codigo_fabricante   = $linhaArray[1];
            ...
            $id_produto = $linhaArray[4];
            $produto_filial = ProdutoFilial::find() ->joinWith('produto', true, 'INNER JOIN')
                                                    ->where("filial_id = 43 and (codigo_fabricante like '".$codigo_fabricante."' or codigo_fabricante like '0".$codigo_fabricante."')") ->orderBy("produto.id ASC")
                                                    ->one();
