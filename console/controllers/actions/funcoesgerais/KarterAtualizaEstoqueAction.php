...
class KarterAtualizaEstoqueAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_23-10-2020_karter1.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_produtos_23-10-2020_karter1.csv")){
            unlink("/var/tmp/log_produtos_23-10-2020_karter1.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_23-10-2020_karter1.csv", "a");
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as informações de preços a subir
            ...
            echo "\n".$i." - ".$linhaArray[0]." - ".$linhaArray[1]." - ".$linhaArray[3]; //Exibe no console(Terminal) as informações dos preços durante o processamento
            ...
            $preco_compra   = (float) str_replace(",",".",str_replace(".","",$linhaArray[4]));
