...
class ImportacaoLNGAction extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        $file = fopen('/var/tmp/PRODUTOS_LNG_PROCESSADOS.csv', 'r');
        ...
        if (file_exists("/var/tmp/log_importar_precos_lng.csv")){
            unlink("/var/tmp/log_importar_precos_lng.csv");
        }
        
        $arquivo_log = fopen("/var/tmp/log_importar_precos_lng.csv", "a");
        // Escreve no log
        ...
        foreach ($LinhasArray as $k => &$linhaArray ){
            ...
            if ($linhaArray[0] == "CODIGO"){
                continue;
            }
            
            //print_r($linhaArray); echo "\n";
            $codigo_global = str_replace(" ","",str_replace('-','',$linhaArray[16]));
            
            echo $k." - ".$codigo_global."\n";
            if (!Subcategoria::findOne(['id'=>(int)$linhaArray[21]])){
                continue;
            }
            ...
            if ($codigo_global == null and $codigo_global == ""){
                ...
                $produto = new Produto();
                $produto->codigo_fabricante = $linhaArray[19];
                $produto->codigo_global     = $codigo_global;
                $produto->codigo_similar    = $linhaArray[17];
                $produto->nome              = $linhaArray[11];
                $produto->aplicacao         = $linhaArray[22];
                $produto->peso              = $linhaArray[12];
                $produto->altura            = $linhaArray[13];
                $produto->largura           = $linhaArray[14];
                $produto->profundidade      = $linhaArray[15];
                $produto->subcategoria_id   = $linhaArray[21];
                $produto->fabricante_id     = 65;
                //var_dump($produto->save()); die;
                ...
                $valorProdutoFilial->valor              = (float)str_replace(",",".",$linhaArray[27]);
                $valorProdutoFilial->valor_cnpj         = (float)str_replace(",",".",$linhaArray[27]);
                ...
                $caminhoImagem          = "/var/tmp/LNG/fotos_com_logo/".$linhaArray[0].".jpg";
                $caminhoImagemSemLogo   = "/var/tmp/LNG/fotos_sem_logo/".$linhaArray[0].".jpg";
