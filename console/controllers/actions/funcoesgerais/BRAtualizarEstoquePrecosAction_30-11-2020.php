...
class BRAtualizarEstoquePrecosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/br_preco_estoque_caixa_23-11-2020_precificado.csv", 'r');

        while (($line = fgetcsv($file,null,';')) !== false)
        {
            $LinhasArray[] = $line;
            //Cria um array com dados da planilha e indice sendo o codigo_fabricante
            $codigos_produtos[$line[15]] = $line[15];

        }
        fclose($file);

        //Verifica se já existe o arquivo de log da função, exclui caso exista
        if (file_exists("/var/tmp/log_br_preco_estoque_caixa_23-11-2020_precificado.csv")){
            unlink("/var/tmp/log_br_preco_estoque_caixa_23-11-2020_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_br_preco_estoque_caixa_23-11-2020_precificado.csv", "a");
        //Escreve no log
	    ...
	    fwrite($arquivo_log, "\n".'"'.$linhaArray[0].'";"'.$linhaArray[1].'";"'.$linhaArray[2].'";"'.$linhaArray[3].'";"'.$linhaArray[4].'";"'.$linhaArray[5].'";"'.$linhaArray[6].'";"'.$linhaArray[7].'";"'.$linhaArray[8].'";"'.$linhaArray[9].'";"'.$linhaArray[10].'";"'.$linhaArray[11].'";"'.$linhaArray[12].'";"'.$linhaArray[13].'";"'.$linhaArray[14].'";"'.$linhaArray[15].'";');
            ...
            echo "\n".$i." - ".$linhaArray[15]." - ".$linhaArray[7]." - ".$linhaArray[13]." - ".$linhaArray[8]; //Exibe no console(Terminal) as informações dos preços durante o processamento
        ...
        foreach ($LinhasArray as &$linhaArray){
            ...
            if ( $codigo_forncedor == null or  $codigo_forncedor == "" or  $codigo_forncedor == "id"){
                ...
                $produto = Produto::find()  ->andWhere(["=","codigo_fabricante","CX.".$linhaArray[1].".B"])
                    ->andWhere(["=","fabricante_id",52])
                    ->one();
                ...
                if (isset($produto)){
                    ...
                    // Escreve no log
                    fwrite($arquivo_log, $linhaArray[1].";".$linhaArray[13].";Produto não encontrado\n");
                }
            }
        }
