...
class BRAtualizarMarcasAction extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        $file = fopen("/var/tmp/br_marcas.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_br_marcas.csv")){
            unlink("/var/tmp/log_br_marcas.csv");
        }
        $arquivo_log = fopen("/var/tmp/log_br_marcas.csv", "a");
        ...
        foreach ($LinhasArray as $i => &$linhaArray){
            ...
            $produto = Produto::find()  ->andWhere(['=','codigo_fabricante', $linhaArray[0].".B"])
