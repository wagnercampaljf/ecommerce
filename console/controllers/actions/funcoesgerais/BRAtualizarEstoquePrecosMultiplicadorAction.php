...
class BRAtualizarEstoquePrecosMultiplicadorAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_br_não_cadastrados ATUALIZADO 27-01.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_produtos_br_não_cadastrados ATUALIZADO 27-01.csv")){
            unlink("/var/tmp/log_produtos_br_não_cadastrados ATUALIZADO 27-01.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_br_não_cadastrados ATUALIZADO 27-01.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as &$linhaArray){
            ...
            if ( $codigo_forncedor == null or  $codigo_forncedor == "" or  $codigo_forncedor == "id"){
                ...
                if (isset($produto)){
                    ...
                    $produto->nome                     = $linhaArray[17];

                    $produto->aplicacao                = $linhaArray[16];

                    $produto->descricao                = $linhaArray[17];

                    $produto->save();

                    fwrite($arquivo_log, $linhaArray[1].";".$linhaArray[15].";Nome Atualizado\n");
                   echo " - Nome Atualizado";
                }
                else {
                    // Escreve no log
                    fwrite($arquivo_log, $linhaArray[1].";".$linhaArray[15].";Nome não Atualizado\n");
                    echo " - Nome não Atualizado";
                }
            }
        }
