...
class BRCriarProdutosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_forros.csv", 'r'); //Abre arquivo com preços para subir
        ...
        if (file_exists("/var/tmp/log_produtos_forros.csv")){
            unlink("/var/tmp/log_produtos_forros.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_forros.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            if (!$produto){ //
                ...
                $produto_novo->nome                 = $linhaArray[0];
                $produto_novo->peso                 = 5;
                ...
                $produto_novo->descricao            = $linhaArray[0]." ".$linhaArray[1];
