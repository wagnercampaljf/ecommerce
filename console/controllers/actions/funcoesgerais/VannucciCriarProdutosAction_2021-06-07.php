...
class VannucciCriarProdutosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/vannucci_novos_produtos-31-05-2021_precificado.csv", 'r'); //Abre arquivo com preços para subir
        ...
        if (file_exists("/var/tmp/log_vannucci_novos_produtos-31-05-2021_precificado.csv")){
            unlink("/var/tmp/log_vannucci_novos_produtos-31-05-2021_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_vannucci_novos_produtos-31-05-2021_precificado.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            if (!$produto){ //Se encontrar o produto, processa o preço
                ...
                $produto_novo->nome                 = strtoupper ( $linhaArray[3]);
                $produto_novo->peso                 = 5;
