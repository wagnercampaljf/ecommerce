...
class AtualizarCodigoGlobalAction extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        $file = fopen('/var/tmp/produtos_DIB-23-02-2021.csv', 'r');
        ...
        if (file_exists("/var/tmp/log_produtos_DIB-23-02-2021.csv")){
            unlink("/var/tmp/log_produtos_DIB-23-02-2021.csv");
        }


        $arquivo_log = fopen("/var/tmp/log_produtos_DIB-23-02-2021.csv", "a");
        // Escreve no log
        ...
        foreach ($LinhasArray  as  $k => &$linhaArray){
            ...
            if ($produtoFilial) {
                ...
                if ($produto){
                    ...
                    $produto->nome = str_replace("DIB","", $linhaArray[2]);
