...
class AtualizarCodigoFabricanteBonfanteAction extends Action
{
    public function run(){
        ...
        $LinhasArray = Array();
        $file = fopen('/var/tmp/planilha_bonfante_codigos.csv', 'r');
        ...
        if (file_exists("/var/tmp/log_planilha_bonfante_codigos.csv")){
            unlink("/var/tmp/log_planilha_bonfante_codigos.csv");
        }

        $arquivo_log = fopen("/var/tmp/log_planilha_bonfante_codigos.csv", "a");
        // Escreve no log
        ...
        foreach ($LinhasArray  as  $k => &$linhaArray){
            ...
            if ($produto){
                ...
                if($produto->codigo_fabricante == $linhaArray[0]){
