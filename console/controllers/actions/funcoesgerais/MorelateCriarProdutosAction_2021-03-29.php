...
class MorelateCriarProdutosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/morelate_novos_23-02-2021.csv", 'r'); //Abre arquivo com preços para subir
        ...
        if (file_exists("/var/tmp/log_morelate_novos_23-02-2021.csv")){
            unlink("/var/tmp/log_morelate_novos_23-02-2021.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_morelate_novos_23-02-2021.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            if (!$produto){ //
               ...
               // $subcategoria = Subcategoria::find()->andWhere(['=','meli_id',446])->one();
                ...
                $produto_novo->codigo_global        = $linhaArray[4];
                $produto_novo->codigo_fabricante    = $linhaArray[0].'.M';
                $produto_novo->nome                 = strtoupper ( $linhaArray[2] );
               ...
               // $produto_novo->codigo_montadora     = $linhaArray[6];
                $produto_novo->codigo_barras        = $linhaArray[10];
