...
class BRAtualizarEstoquePrecosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/br_preco_estoque_24-09-2020_precificado.csv", 'r');

        while (($line = fgetcsv($file,null,';')) !== false)
        {
            $LinhasArray[] = $line;
            $codigos_produtos[$line[0].".B"] = $line[0];

        }
        fclose($file);

        //Verifica se já existe o arquivo de log da função, exclui caso exista
        if (file_exists("/var/tmp/log_br_preco_estoque_24-09-2020_precificado.csv")){
            unlink("/var/tmp/log_br_preco_estoque_24-09-2020_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_br_preco_estoque_24-09-2020_precificado.csv", "a");
        //Escreve no log
	    ...
	    fwrite($arquivo_log, "\n".'"'.$linhaArray[0].'";"'.$linhaArray[1].'";"'.$linhaArray[2].'";"'.$linhaArray[3].'";"'.$linhaArray[4].'";"'.$linhaArray[5].'";"'.$linhaArray[6].'";"'.$linhaArray[7].'";"'.$linhaArray[8].'";"'.$linhaArray[8].'";"'.$linhaArray[9].'";"'.$linhaArray[10].'";"'.$linhaArray[11].'";"'.$linhaArray[12].'";"'.$linhaArray[13].'";"'.$linhaArray[14].'";');
        ...
        foreach($produtos_filial_br as $k => $produto_filial_br){
            ...
            if(array_key_exists($produto_filial_br->codigo_fabricante, $codigos_produtos)){
                $produto_encontrado = true;
            }

            if(!$produto_encontrado){
                echo "\n".$k." - ".$produto_filial_br->codigo_fabricante." - produto não encontrado na planilha";
                ...
                if($produto_filial){
                    ...
                    if($valor_produto_filial){
                        ...
                        // }
                        fwrite($arquivo_log, "\n".'"'.$produto_filial_br->id.'";"'.$produto_filial_br->codigo_fabricante.'";"'.$quantidade.'";"'.$status.'"');
                    }
                }
            }
        }
