...
class BonfanteAtualizarPrecosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_bonfante_18-11-2020_import_precificado.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_produtos_bonfante_18-11-2020_import_precificado.csv")){
            unlink("/var/tmp/log_produtos_bonfante_18-11-2020_import_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_produtos_bonfante_18-11-2020_import_precificado.csv", "a");
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            echo "\n".$i." - " .$linhaArray[0]." - ".$linhaArray[5]." - ".$linhaArray[6]." - ".$linhaArray[8]; //Exibe no console(Terminal) as informações dos preços durante o processamento
