...
class BRCriarProdutosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/produtos_caixas_lng.csv", 'r'); //Abre arquivo com preços para subir
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            if ($produto){ //
                ...
                $produto_novo->codigo_global        = "CX.".$linhaArray[0];
                $produto_novo->codigo_fabricante    = "CX.".$linhaArray[1];
                $produto_novo->nome                 = strtoupper ( "CAIXA ".$linhaArray[4]);
                $produto_novo->peso                 = 5;
                $produto_novo->altura               = 30;
                $produto_novo->largura              = 30;
                $produto_novo->profundidade         = 30;
                $produto_novo->subcategoria_id      = 95;
                $produto_novo->produto_condicao_id   = 1;
                $produto_novo->aplicacao            = $linhaArray[2];
                $produto_novo->multiplicador            = $linhaArray[5];
                ...
                $produto_filial_novo->filial_id                     = 60;
