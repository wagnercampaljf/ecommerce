...
class BRAtualizarEstoquePrecosAction extends Action
{
    public function run(){
        ...
        //$file = fopen("/var/tmp/br_preco_estoque_10-12-2020_precificado.csv", 'r');
        // $file = fopen("/var/tmp/br_preco_estoque_11-12-2020_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_16-12-2020_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_30-12-2020_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_12-01-2021_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_18-01-2021_precificado.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_br_preco_estoque_17-03-2021_precificado.csv")){
            unlink("/var/tmp/log_br_preco_estoque_17-03-2021_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_br_preco_estoque_17-03-2021_precificado.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            echo "\n".$i." - ".$linhaArray[14]." - ".$linhaArray[7]." - ".$linhaArray[14]." - ".$linhaArray[8]; //Exibe no console(Terminal) as informações dos preços durante o processamento
