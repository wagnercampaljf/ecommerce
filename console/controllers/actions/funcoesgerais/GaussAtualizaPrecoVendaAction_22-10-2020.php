...
class GaussAtualizaPrecoVendaAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/gauss_preco_estoque_24-09-2020_precificado.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_gauss_preco_estoque_24-09-2020_precificado.csv")){
            unlink("/var/tmp/log_gauss_preco_estoque_24-09-2020_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_gauss_preco_estoque_24-09-2020_precificado.csv", "a");
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as informações de preços a subir
            ...
            fwrite($arquivo_log, "\n".$linhaArray[0].';'.$linhaArray[1].';'.$linhaArray[2].';'.$linhaArray[3].';'.$linhaArray[4].';'.$linhaArray[5].";".$linhaArray[6].";".$linhaArray[7].";".$linhaArray[8].";".$linhaArray[9].";".$linhaArray[10].";".$linhaArray[11].";".$linhaArray[12].";".$linhaArray[13].";".$linhaArray[14].";".$linhaArray[15].";".$linhaArray[16].";".$linhaArray[17].";".$linhaArray[18]);
            ...
            if ($produto){ //Se encontrar o produto, processa o preço
                ...
                $produto_filial = ProdutoFilial::find()->andWhere(['=','produto_id',$produto->id])->andWhere(['=','filial_id',96])->one();
