...
class BRAtualizarEstoquePrecosAction extends Action
{
    public function run(){
        ...
        //$file = fopen("/var/tmp/br_preco_estoque_03-02-2021_precificado.csv", 'r');
        // $file = fopen("/var/tmp/br_atualizar.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_25-02-2021_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_09-03-2021_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_17-03-2021_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_18-03-2021_precificado.csv", 'r');
        //$file = fopen("/var/tmp/br_preco_estoque_22-03-2021_precificado.csv", 'r');
       ...
       // $file = fopen("/var/tmp/br_preco_estoque_20-04-2021_precificado.csv", 'r');
        ...
        if (file_exists("/var/tmp/log_br_preco_estoque_26-04-2021_precificado.csv")){
            unlink("/var/tmp/log_br_preco_estoque_26-04-2021_precificado.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_br_preco_estoque_26-04-2021_precificado.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            if ($produto){ //Se encontrar o produto, processa o preço
                ...
                if ($produtoFilial) {//Se encontrar estoque, processa
                    ...
                    if($linhaArray[15]== '004760.B' && $linhaArray[15]== '509371.B'){
                        $quantidade = 0;
