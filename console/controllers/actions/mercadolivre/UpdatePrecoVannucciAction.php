...
class UpdatePrecoVannucciAction extends Action
{
    public function run()
    {
        $nome_arquivo = "/var/tmp/log_update_preco_ml_Vannucci_".date("Y-m-d_H-i-s").".csv";
        ...
        $filial = Filial::find()->andWhere(['=','id',38])->one();
        ...
        foreach ($filials as $filial) {
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
        	    ...
        	    if($produto_filial->filial_id == 38 && $k <= 33211){
                        continue;
        	    }
