...
class UpdatePrecoFisicaAction extends Action
{
    public function run()
    {
        $nome_arquivo = "/var/tmp/log_update_preco_ml_Fisica_".date("Y-m-d_H-i-s").".csv";
        ...
        foreach ($filials as $filial) {
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
                ...
                foreach ($produto_filiais as $k => $produto_filial) {
        		    ...
        		    if($produto_filial->filial_id == 96 && $k <= 3940){
                        continue;
        		    }
