...
class PedidosCriarAlterarAction extends Action
{
    ...
    public function run(){
        ...
        // Escreve no log
        $arquivo_log = fopen("/var/tmp/pedidos_criar_alterar_".date("Y-m-d_H-i-s").".csv", "a");
        ...
        if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
            ...
            $data_inicial = date('Y-m-d', strtotime("-10 days",strtotime(date('Y-m-d')))); 
...
continue;
                        //$pedido_pecaagora_ml = PedidoMercadoLivre::find()->andWhere(['=', 'pedido_meli_id', $venda->id])->one();
                        
                        //if($pedido_pecaagora_ml){
                        //    continue;
                        //}
                        
                        //if($venda->id != "4418049554"){
                        //    continue;
                        //}
                        ...
                        //INICIO
                        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                        $e_pedido_criado_alterado_peca_agora = $this->criarPedidoMercadoLivre($venda);
                        $envio_dados = $meli->get("/shipments/".ArrayHelper::getValue($venda, 'shipping.id')."?access_token=" . $meliAccessToken);
                        if ($envio_dados['httpCode'] >= 300) {
                            echo "Sem informações de envio";
                            continue;
                        }
                        else {
                            $e_pedido_criado_alterado_peca_agora = $this->criarPedidoMercadoLivre($venda, $envio_dados);
                        }
                        
                        $codigo_cenario_impostos = "2602379535";
                         
                        $e_pedido_criado_alterado_omie = $this->criarPedidoOmie($venda, $envio_dados, $codigo_cenario_impostos, $APP_KEY_OMIE_SP, $APP_SECRET_OMIE_SP, 505552563, 500726231, 502875713);
            ...
            while (ArrayHelper::getValue($response_order, 'body.results') != null){
                ...
                if($x>=0){
                    foreach (ArrayHelper::getValue($response_order, 'body.results') as $k => $venda){
                        ...
                        echo "\n"; print_r($venda->buyer->billing_info->doc_number);echo " - "; print_r($venda->date_created); echo " - "; print_r($venda->id); echo " - "; print_r($venda->buyer->first_name); echo " - ".ArrayHelper::getValue($venda, 'shipping.id');
continue;
                        /*$pedido_pecaagora_ml = PedidoMercadoLivre::find()->andWhere(['=', 'pedido_meli_id', $venda->id])->one();
                        ...
                        $e_pedido_criado_alterado_omie = $this->criarPedidoOmie($venda, $envio_dados, $codigo_cenario_impostos, $APP_KEY_OMIE_CONTA_DUPLICADA, $APP_SECRET_OMIE_CONTA_DUPLICADA, 1018250911, 1018256043, 1018255531);
    ...
    public function criarPedidoOmie($response_order, $envio_dados, $codigo_cenario_impostos, $APP_KEY_OMIE, $APP_SECRET_OMIE, $codigo_transportadora, $codVend, $codigo_conta_corrente){
        ...
        $body = [
            ...
            "param" => [
                "codigo_cliente_integracao" => ArrayHelper::getValue($response_order, 'buyer.id'),
            ]
        ];
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200){
            ...
            $body = [
                ...
                "app_secret" => $APP_SECRET_OMIE,
                "param" => [
                    "codigo_cliente_integracao" => substr(ArrayHelper::getValue($response_order, 'buyer.id'),0,20),
                    "razao_social"              => str_replace(" ","%20",substr(ArrayHelper::getValue($response_order, 'buyer.first_name')." ".ArrayHelper::getValue($response_order, 'buyer.last_name'), 0, 60)),
                    "cnpj_cpf"                  => substr(ArrayHelper::getValue($response_order, 'buyer.billing_info.doc_number'),0,20),
                    "nome_fantasia"             => str_replace(" ","%20",substr(ArrayHelper::getValue($response_order, 'buyer.first_name')." ".ArrayHelper::getValue($response_order, 'buyer.last_name'),0,100)),
                    ...
                    "email"                     => "cliente.pecaagora@gmail.com",//ArrayHelper::getValue($response_order, 'buyer.email'),
                ]
            ];
        ...
        $body = [
            ...
            "app_secret" => $APP_SECRET_OMIE,
            "param" => [
                "codigo_pedido_integracao" => ArrayHelper::getValue($response_order, 'id'),
            ]
        ];
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200){
            ...
            $produtoML = ProdutoFilial::find()->andWhere(['=','meli_id',ArrayHelper::getValue($response_order, 'order_items.0.item.id')])->one();

            if(!$produtoML){
                $produtoML = ProdutoFilial::find()->andWhere(['=','meli_id_sem_juros',ArrayHelper::getValue($response_order, 'order_items.0.item.id')])->one();
                
                if(!$produtoML){
                    $produtoML = ProdutoFilial::find()->andWhere(['=','meli_id_full',ArrayHelper::getValue($response_order, 'order_items.0.item.id')])->one();
                }
            }
            
            if(!$produtoML){
                echo " - PRODUTO SEM VÍNCULO";
                return null;
            }
	    $codigo_pa      = "PA".$produtoML->produto_id;
            ...
            $quantidade     = ArrayHelper::getValue($response_order, 'order_items.0.quantity') * $multiplicador;
            $valor_unitario = ArrayHelper::getValue($response_order, 'order_items.0.unit_price')/$multiplicador;
            ...
            $body = [
                ...
                "param" => [
                    "cabecalho" => [
                        ...
                        "codigo_pedido_integracao"  => ArrayHelper::getValue($response_order, 'id'),
                        "etapa"                     => "10",
                        "data_previsao"             => substr(ArrayHelper::getValue($response_order, 'date_created'),8,2).'/'.substr(ArrayHelper::getValue($response_order, 'date_created'),5,2).'/'.substr(ArrayHelper::getValue($response_order, 'date_created'),0,4),
                        "quantidade_itens"          => ArrayHelper::getValue($response_order, 'order_items.0.quantity'),
                    ],
                    "det"=> [
                        "ide"=> [
                        ...
                        "produto" => [
                            ...
                            "cfop"                      => $cfop,
                            "quantidade"                => $quantidade,//ArrayHelper::getValue($response_order, 'order_items.0.quantity'),
                            "valor_unitario"            => $valor_unitario,//ArrayHelper::getValue($response_order, 'order_items.0.unit_price'),
                        ],
                    ],
                    "frete" => [
                    ...
                    "informacoes_adicionais"    => [
                        "numero_contrato"           => ArrayHelper::getValue($response_order, 'payments.0.id'),
                        "numero_pedido_cliente"     => ArrayHelper::getValue($response_order, 'id'),
                        "consumidor_final"          => "S",
    ...
    public function criarPedidoMercadoLivre($order = null, $shipping = null){
        ...
        $pedido_mercado_livre = PedidoMercadoLivre::find()->andwhere(['=', 'pedido_meli_id', ArrayHelper::getValue($order, 'id')])->one();

        if($pedido_mercado_livre){
            $pedido_mercado_livre->pedido_meli_id       = (string) ArrayHelper::getValue($order, 'id');
            $pedido_mercado_livre->total_amount         = ArrayHelper::getValue($order, 'total_amount');
            $pedido_mercado_livre->date_created         = str_replace("T", " ",substr(ArrayHelper::getValue($order, 'date_created'),0,19));
            $pedido_mercado_livre->date_closed          = str_replace("T", " ",substr(ArrayHelper::getValue($order, 'date_closed'),0,19));
            $pedido_mercado_livre->last_updated         = str_replace("T", " ",substr(ArrayHelper::getValue($order, 'last_updated'),0,19));
            $pedido_mercado_livre->paid_amount          = (string) ArrayHelper::getValue($order, 'paid_amount');
            $pedido_mercado_livre->shipping_id          = (string) ArrayHelper::getValue($order, 'shipping.id');
            $pedido_mercado_livre->status               = (string) ArrayHelper::getValue($order, 'status');
            $pedido_mercado_livre->buyer_id             = (string) ArrayHelper::getValue($order, 'buyer.id');
            $pedido_mercado_livre->buyer_nickname       = (string) (isset($order->buyer->nickname)) ? ArrayHelper::getValue($order, 'buyer.nickname') : "";
            $pedido_mercado_livre->buyer_email          = (string) (isset($order->buyer->email)) ? ArrayHelper::getValue($order, 'buyer.email') : "";
            $pedido_mercado_livre->buyer_first_name     = (string) (isset($order->buyer->first_name)) ? ArrayHelper::getValue($order, 'buyer.first_name') : "";
            $pedido_mercado_livre->buyer_last_name      = (string) (isset($order->buyer->last_name)) ? ArrayHelper::getValue($order, 'buyer.last_name') : "";
            $pedido_mercado_livre->buyer_doc_type       = (string) (isset($order->buyer->billing_info->doc_type)) ? ArrayHelper::getValue($order, 'buyer.billing_info.doc_type') : "";
            $pedido_mercado_livre->buyer_doc_number     = (string) (isset($order->buyer->billing_info->doc_number)) ? ArrayHelper::getValue($order, 'buyer.billing_info.doc_number') : "";
            $pedido_mercado_livre->user_id              = (string) ArrayHelper::getValue($order, 'seller.id');
            $pedido_mercado_livre->pack_id              = (string) ArrayHelper::getValue($order, 'pack_id');
            ...
            $pedido_mercado_livre->buyer_nickname       = (string) (isset($order->buyer->nickname)) ? ArrayHelper::getValue($order, 'buyer.nickname') : "";
            $pedido_mercado_livre->buyer_email          = (string) (isset($order->buyer->email)) ? ArrayHelper::getValue($order, 'buyer.email') : "";
            $pedido_mercado_livre->buyer_first_name     = (string) ArrayHelper::getValue($order, 'buyer.first_name');
            $pedido_mercado_livre->buyer_last_name      = (string) ArrayHelper::getValue($order, 'buyer.last_name');
            $pedido_mercado_livre->buyer_doc_type       = (string) ArrayHelper::getValue($order, 'buyer.billing_info.doc_type');
            $pedido_mercado_livre->buyer_doc_number     = (string) ArrayHelper::getValue($order, 'buyer.billing_info.doc_number');
            $pedido_mercado_livre->user_id              = (string) ArrayHelper::getValue($order, 'seller.id');
            $pedido_mercado_livre->pack_id              = (string) ArrayHelper::getValue($order, 'pack_id');
        ...
        if($pedido_mercado_livre){
            ...
            if(!is_null($shipping)){
                ...
                $pedido_mercado_livre->shipping_date_created                = str_replace("T", " ",substr(ArrayHelper::getValue($order, 'date_created'),0,19));
                $pedido_mercado_livre->shipping_last_updated                = str_replace("T", " ",substr(ArrayHelper::getValue($order, 'last_updated'),0,19));
                ...
                if($pedido_mercado_livre->save()){
                    $e_pedido_criado_alterado = true;
                    echo " - Shipping alterado";
                }
                else{
                    $e_pedido_criado_alterado = true;
                    echo " - Shipping não alterado";
                }
                ...
                if($pedido_mercado_livre_shipments){
                    echo " - Dados shipments já cadastrados";
                    ...
                    if($pedido_mercado_livre_shipments->save()){
                        ...
                        foreach(ArrayHelper::getValue($shipping, 'body.shipping_items') as $shipping_item){
                            ...
                            if($pedido_mercado_livre_shipments_item->save()){
                                echo " - Shipping Item ".$status_item;
                            }
                            else{
                                echo " - Shipping Item não ".$status_item;
                            }
                        }
                    }
                    else{
                        $e_pedido_criado_alterado = false;
                        echo " - Shipping tabela não alterado";
                    }
                }
                else{
                    echo " - Dados shipments não cadastrados";
                    ...
                    if($pedido_mercado_livre_shipments->save()){
                        ...
                        $e_shipping_criado_alterado = false;
                        echo " - Shipping tabela não criado";
...
Skype: pecaagora";
            ...
            //print_r($order);            
            foreach(ArrayHelper::getValue($order, 'order_items') as $k => $produto){
            ...
            foreach(ArrayHelper::getValue($order, 'payments') as $k => $pagamento){
                ...
                if($pedido_mercado_livre_pagamento){
                    ...
                    if($pedido_mercado_livre_pagamento->save()){
                        ...
                        echo " - Pagamento não criado";
                    }
                }
            }
            
            $pedido_mercado_livre->email_texto = $produtos_email;
            $pedido_mercado_livre->email_assunto = "Pedido {codigo_fabricante} * {quantidade} - {nome} ({margem})";
            if($pedido_mercado_livre->user_id == "435343067"){
                $pedido_mercado_livre->email_assunto = "Novo Pedido {codigo_fabricante} * {quantidade} - {nome} ({margem})";
            }
