        ...
        foreach ($filials as $filial) {
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
                ...
                $produtoFilials = $filial   ->getProdutoFilials()
                                            ...
                                            //->andWhere(['produto_filial.meli_id' => ['MLB1245241465']])
			...
			//print_r(count($produto_frete));die;
                        ...
                        if ($response['httpCode'] >= 300) {
                            ...
                            foreach ($produtos_filiais_outros as $produto_filial_outro){
                                ...
                                if (ArrayHelper::keyExists($produto_filial_outro->meli_id, $produto_frete, false)){
                                    if($preco>=510){
                                        $preco_outro += round((ArrayHelper::getValue($produto_frete, $produto_filial_outro->meli_id)/2), 2);
                                        $preco_outro = $preco_outro-10;
                                    }
                                    elseif($preco > 120 && $preco < 510){
                                        $preco_outro += round((ArrayHelper::getValue($produto_frete, $produto_filial_outro->meli_id)/2), 2);
                                        $preco_outro =  $preco_outro-16;
                                    }
                                }
