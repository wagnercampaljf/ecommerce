...
class GerarRelatorioFreteGratisValorMenor100Action extends Action
{
    ...
    public function run($cliente = 1){
        ...
        if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
            ...
            //$response_itens = $meli->get("/sites/MLB/search?seller_id=193724256&shipping_cost=free&orders=price_asc&access_token=" . $meliAccessToken);
            ...
            while (ArrayHelper::getValue($response_itens, 'httpCode') <> 404){
                ...
                if ($i >= 0){
                    foreach (ArrayHelper::getValue($response_itens, 'body.results') as $meli_id){
                        ...
                        if($x < 44000){ //82290 - principal //83604 - Conta Duplicada 
                            echo " - pular";
                            continue;
                        }
                ...
                echo "\n Scroll: ".$i++;
                $response_itens = $meli->get("/users/193724256/items/search?search_type=scan&status=active&shipping_cost=free&scroll_id=".ArrayHelper::getValue($response_itens, 'body.scroll_id')."&limit=100&access_token=" . $meliAccessToken);
