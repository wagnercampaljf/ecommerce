...
class VerificacaoCaixasAction extends Action
{
    ...
    public function run($cliente = 1){
        ...
        // Escreve no log
        $arquivo_log = fopen("/var/tmp/verificacao_caixas_BR_".date("Y-m-d_H-i-s").".csv", "a");
        ...
        fwrite($arquivo_log, "filial_id;filial_nome;produto_filial_id;produto_id;produto_nome;codigo_global;codigo_fabricante;quantidade;status;");
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
                ...
                $produtos_filiais = ProdutoFilial::find()->joinWith("produto")
                                                        //  ->andWhere(['like','codigo_global','CX-'])
                                                         ->andWhere(['like','codigo_global','%CX%',false])
                ...
                foreach ($produtos_filiais as $i => $produto_filial){
                    ...
                    if($response_item["httpCode"] < 300 ){
                        ...
                        fwrite($arquivo_log, "\n".$filial->id.";".$filial->nome.";".$produto_filial->id.";".$produto_filial->produto_id.";".$produto_filial->produto->nome.";".$produto_filial->produto->codigo_global.";".$produto_filial->produto->codigo_fabricante.";".$produto_filial->quantidade.";".$response_item["body"]->status.";");
                        echo " - OK";

                        if($produto_filial->produto->codigo_global = (substr($produto_filial->produto->codigo_global,0,3)== "CX.")){

                            fwrite($arquivo_log, " Dentro do Padrão");
                            echo " - Dentro do Padrão";
                        }else{
                            fwrite($arquivo_log, " Fora do Padrão");
                            echo " - Fora do Padrão";
                        }
