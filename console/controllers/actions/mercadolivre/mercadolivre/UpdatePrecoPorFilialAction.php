        ...
        foreach ($filials as $filial) {
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
                ...
                $produto_filiais = $filial  ->getProdutoFilials()
                                            ...
                                            ->where(" id = 139640 and meli_id is not null and produto_filial.id in (select distinct produto_filial_id from valor_produto_filial) ")
                                            //->andWhere(['>','quantidade',0])
                                            //->andWhere(['produto_filial.meli_id' => []])
                                            //->andWhere(['produto_filial.meli_id' => ['MLB1528400982']])
                                            //->andWhere(['produto_filial.id' => [314980]])
