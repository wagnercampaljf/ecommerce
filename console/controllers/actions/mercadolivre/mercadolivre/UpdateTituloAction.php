        ...
        foreach ($filials as $filial) {
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
                ...
                $produtoFilials = $filial   ->getProdutoFilials()
                                            ...
                                            //->andWhere(['produto_filial.id' => [10225]])
				...
				fwrite($arquivo_log, "\n".$produtoFilial->id.";".$preco.";Título não alterado");
                            }else {
				fwrite($arquivo_log, "\n".$produtoFilial->id.";".$preco.";Título alterado");
                            }
			    ...
			    //Atualização Imagens
					...
					"pictures" => $produtoFilial->produto->getUrlImagesML(),
                            ...
                            $response = $meli->put("items/{$produtoFilial->meli_id}?access_token=" . $meliAccessToken, $body, [] );
                            if ($response['httpCode'] >= 300) {
                                fwrite($arquivo_log, ";Imagens não ateradas");
                            }else {
                                fwrite($arquivo_log, ";Imagens alteradas");
                            }
