...
class BRCriarProdutosAction extends Action
{
    public function run(){
        ...
        $file = fopen("/var/tmp/br_ausente_completo_com_medidas_16-04-2020.csv", 'r'); //Abre arquivo com preços para subir
        ...
        if (file_exists("/var/tmp/log_br_ausente_completo_com_medidas_16-04-2020.csv")){
            unlink("/var/tmp/log_br_ausente_completo_com_medidas_16-04-2020.csv");
        }

        //Abre o arquivo de log pra ir logando a função
        $arquivo_log = fopen("/var/tmp/log_br_ausente_completo_com_medidas_16-04-2020.csv", "a");
        //Escreve no log
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            if (!$produto){ //Se encontrar o produto, processa o preço
                ...
                $produto_novo->nome                 = $linhaArray[12];
                $produto_novo->peso                 = $linhaArray[18]/1000;
                $produto_novo->altura               = $linhaArray[19];
                $produto_novo->largura              = $linhaArray[20];
                $produto_novo->profundidade         = $linhaArray[21];
                $produto_novo->subcategoria_id      = $subcategoria->id;
                $produto_novo->aplicacao            = $linhaArray[13];
                $produto_novo->codigo_montadora     = $linhaArray[2];
                $produto_novo->codigo_barras        = $linhaArray[8];
                $produto_novo->descricao            = $linhaArray[13];
