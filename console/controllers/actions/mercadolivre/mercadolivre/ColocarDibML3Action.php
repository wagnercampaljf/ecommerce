        ...
        $response_outro = ArrayHelper::getValue($user_outro, 'body');
            ...
            foreach($produtos_filial_dib as $k => $produto_filial_dib){
                ...
                if($produtos_filial_dib_duplicada){
                    echo " - Produto encontrado - ".$produtos_filial_dib_duplicada->meli_id;
                    ...
                    if($produtos_filial_dib_duplicada->meli_id != "" and $produtos_filial_dib_duplicada->meli_id != null){
                        ...
                        $produtos_filial_dib_ml3_duplicada = ProdutoFilial::find()  ->andWhere(['=','produto_filial_origem_id',$produto_filial_dib->id])
                                                                                    ->andWhere(['=','filial_id',98])
                                                                                    ->one();
                        
                        if($produtos_filial_dib_ml3_duplicada ){
                            echo " - Produto da conta nova ML 3";
                            $body = [
                            ...
                            if ($response['httpCode'] < 300) {
                                $produtos_filial_dib_ml3_duplicada->meli_id = $response['body']->id;
                                $produtos_filial_dib_ml3_duplicada->save();
                                fwrite($arquivo_log, $produtos_filial_dib_ml3_duplicada->id.";".$preco_outro.";Produto duplicado criado");
                            }
                            else {
