...
print_r($response_order);
	       while (ArrayHelper::getValue($response_order, 'httpCode') < 300){
echo "(1)";
...
echo "(2)";
			//if($y>=10147) print_r($venda);

	               $y++;
                       echo "\n".$x." - ".$y." - ";print_r(ArrayHelper::getValue($venda, 'id'));
echo "(3)";
                       if($y <= 10160){
                               continue;
                       }

	               $response_shipping = $meli->get("/shipments/".ArrayHelper::getValue($venda, 'shipping.id')."?access_token=" . $meliAccessToken);
//print_r($response_shipping);die;
	               ...
	               if(ArrayHelper::getValue($response_shipping, 'body.mode')=='me2'){
	                   /*$y++;
	                   ...
	                   foreach(ArrayHelper::getValue($response_shipping, 'body.shipping_items') as $itens){
	                       ...
	                       if(isset($produto_filial)){
	                           ...
	                           $produto = Produto::find()  ->andWhere(['=','id',$produto_filial->produto_id])
	                                                       ->andWhere(['not like', 'descricao', '(Medidas Conferidas)'])
	                                                       ->one();
	                           if($produto)
	                           {
	                               echo "\nProduto Encontrado!";
...
echo "(5)";
	           $response_order = $meli->get('https://api.mercadolibre.com/orders/search?seller=193724256&offset='.$x.'&order.date_created.from=2017-01-01T00:00:00.000-00:00&order.date_created.to='.$data_atual.'T23:59:59.000-00:00&access_token=' . $meliAccessToken);
if ($y>= 10100) print_r($response_order);
echo "(6)";
	       }
