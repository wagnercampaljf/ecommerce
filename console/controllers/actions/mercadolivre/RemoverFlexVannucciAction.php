...
class RemoverFlexVannucciAction extends Action
{
    ...
    public function run($cliente = 1){
        ...
        // Escreve no log
        $arquivo_log = fopen("/var/tmp/remover_flex_vannucci_".date("Y-m-d_H-i-s").".csv", "a");
        ...
        if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
            ...
            $produtos_filiais = ProdutoFilial::find()->andWhere(["=", "filial_id", 38])
                                                     //->andWhere(["=", "id", 325340])
