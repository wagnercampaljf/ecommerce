...
class PedidosCriarAlterarAction extends Action
{
    public function run()
    {
        ...
        foreach ($contas_ml as $k => $conta_ml) {
            print_r($conta_ml);

            // Escreve no log
            $arquivo_log = fopen("/var/tmp/ml_pedidos_criar_alterar/pedidos_criar_alterar_" . $conta_ml["nome_arquivo"] . "_" . date("Y-m-d_H-i-s") . ".csv", "a");
            ...
            if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
                ...
                $url = 'https://api.mercadolibre.com/orders/search?seller=' . $user_id . '&offset=' . $x . '&order.date_created.from=' . $data_inicial . 'T00:00:00.000-00:00&order.date_created.to=' . $data_atual . 'T23:59:59.000-00:00&access_token=' . $meliAccessToken;
                echo "\n\n".$url."\n\n";
                $response_order = $meli->get($url);
                ...
                while (ArrayHelper::getValue($response_order, 'body.results') != null) {
                    ...
                    if ($x >= 0) {
                        foreach (ArrayHelper::getValue($response_order, 'body.results') as $k => $venda) {
                            ...
                            if ($pedido_mercado_livre) {
                                ...
                                echo "(((((((((((!!!))))))))))";
                                echo PedidoMercadoLivre::baixarPedidoML($venda->id, false);
                                echo "(((((((((((111))))))))))";
                    ...
                    $x += 50;
                    $response_order = $meli->get('https://api.mercadolibre.com/orders/search?seller=' . $user_id . '&offset=' . $x . '&order.date_created.from=' . $data_inicial . 'T00:00:00.000-00:00&order.date_created.to=' . $data_atual . 'T23:59:59.000-00:00&access_token=' . $meliAccessToken);
                }
            }
        }
