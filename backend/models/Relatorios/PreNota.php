...
class PreNota extends Padraopdf
{
    ...
    public function setRelatorio($pedido_mercado_livre)
    {
        ...
        $data_array_criacao    = explode(".", $pedido_mercado_livre->date_created);

        $this->pdf->MultiCell(277, 10, "Nº: " . $pedido_mercado_livre->pedido_meli_id, '', 'L', false, 1);
        $this->pdf->SetFont('helvetica', 'N', 14);
        $this->pdf->MultiCell(130, 4, "Conta: " . $conta, '', 'L', false, 0);
        $this->pdf->MultiCell(130, 4, "Envio: " . $tipo_envio, '', 'L', false, 1);
        $this->pdf->MultiCell(130, 4, substr(("Nome: " . $pedido_mercado_livre->buyer_first_name . " " . $pedido_mercado_livre->buyer_last_name), 0, 42), '', 'L', false, 0);
        $this->pdf->MultiCell(130, 4, str_replace(" ", "", $pedido_mercado_livre->buyer_doc_type) . ": " . $pedido_mercado_livre->buyer_doc_number, '', 'L', false, 1);
        $this->pdf->MultiCell(260, 4, "Endereço: " . $pedido_mercado_livre->receiver_street_name . " " . $pedido_mercado_livre->receiver_street_number . ", " . $pedido_mercado_livre->receiver_neighborhood_name . ", " . $pedido_mercado_livre->receiver_comment . ", " . $pedido_mercado_livre->receiver_city_name . ", " . $pedido_mercado_livre->receiver_state_name . " - " . $pedido_mercado_livre->receiver_zip_code, '', 'L', false, 1);
        $this->pdf->MultiCell(130, 4, "Data/Hora Pedido ML: " . $data_array_criacao[0], '', 'L', false, 1);
        $this->pdf->MultiCell(130, 4, "Data/Hora Autorização: " . $data_array[0], '', 'L', false, 0);
        $this->pdf->MultiCell(130, 4, "Autorizado por: " . $administrador->nome, '', 'L', false, 1);
        ...
        foreach ($produtos as $produto) {
            ...
            if ($imagem) {
                ...
                copy($imagem_caminho, '/var/www/html/frontend/web/assets/img/imagens_temporarias/' . $imagem->id . ".webp", stream_context_create($stream_opts));
                ...
                $imagem = $this->pdf->Image($caminho, $this->pdf->getX(), $this->pdf->getY(), 32, 25, 'JPG', $link_produto, '', true, 300, 'J', false, false, 1, false, false, false);
            }
            //$caminho_imagem = "https://www.pecaagora.com/imagens/produto_230615/230615_1.webp";
            //$imagem = $this->pdf->Image($caminho_imagem, $this->pdf->getX(), $this->pdf->getY(), 35, 28, 'WEBP', $link_produto, '', true, 300, 'J', false, false, 1, false, false, false);
            ...
            $this->pdf->MultiCell(260, 6, "Localização: " . $produto["localizacao"], 0, 'L', false, 1);
            $i++;
