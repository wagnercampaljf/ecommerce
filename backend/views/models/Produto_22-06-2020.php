...
class Produto extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['fabricante_id', 'marca_produto_id', 'subcategoria_id', 'quantidade', 'multiplicador', 'envio', 'ordem', 'filial_id'], 'integer'],
            [['nome'], 'string', 'max' => 150],
    ...
    public function attributeLabels()
    {
	    ...
	    'marca_produto_id'          => 'Marca (Produto)',
        ];
    }
...
//        echo Url::to(['/p/' . $this->id . '/' . $this->slug,], $absolute);
	...
	ArrayHelper::multisort($imagens,['ordem'],[SORT_ASC]);
		...
		copy($caminho, '/var/www/html/frontend/web/assets/img/imagens_temporarias/'.$v->id."_".$v->ordem.".webp" );
		...
		$src[] = ['source' => 'https://www.pecaagora.com/frontend/web/assets/img/imagens_temporarias/'.$v->id.'_'.$v->ordem.'.jpg'];
            ...
            $src[] = ['source' => 'https://123'];//'https://www.pecaagora.com/frontend/web/assets/img/produtos/no-image.png'];
        }
        return $src;
    }
...
class ProdutoQuery extends \yii\db\ActiveQuery
{
    ...
    public function byMarcaProduto($marca_produto_id)
    {
        if (is_null($marca_produto_id)) {
            return $this;
        }
        
        return $this->joinWith(['marca_produto'])->andWhere(['marca_produto.id' => $marca_produto_id]);
    }
