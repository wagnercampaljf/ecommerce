 ...
 * @property boolean $e_pedido_enviado
 * @property integer $autorizado_por
 * @property integer $enviado_por
 * @property integer $nota_fiscal_id
 * @property integer $quantidade_impressoes_etiqueta
 * @property integer $quantidade_impressoes_nota_fiscal
 *
...
class PedidoMercadoLivre extends \yii\db\ActiveRecord
{
    ...
    public function attributeLabels()
    {
	    ...
	    'data_hora_autorizacao' => "Data/Hora Autorização",
	    'data_hora_envio' => "Data/Hora Envio",
            'e_pedido_enviado' => "É pedido enviado",
            'autorizado_por' => "Autorizado por",
            'enviado_por' => "Enviado por",
	    'nota_fiscal_id' => "Nota Fiscal",
	    'quantidade_impressoes_etiqueta' => "Quantidade Impressões Etiquetas",
            'quantidade_impressoes_nota_fiscal' => "Quantidade Impressões Etiquetas"
        ];
    }
...
//        echo "<pre>".$order.$url_shipping; print_r($envio_dados); echo "</pre>"; return $status_retorno;
        ...
        $status_retorno .= self::criarPedidoOmie($user_id, $response_order, $envio_dados, $billing_info, $e_html);
    ...
    public static function criarPedidoOmie($user_id, $response_order, $envio_dados, $billing_info, $e_html){
        ...
        $body = [
            ...
            "param" => [
                ...
                "clientesFiltro" => [
                    "cnpj_cpf" => ArrayHelper::getValue($billing_info,"body.billing_info.doc_number"),
                ]
            ]
        ];
        $response_omie = $omie->consulta_cliente("api/v1/geral/clientes/?JSON=",$body);
        //print_r($billing_info["body"]->billing_info->doc_number);
	//print_r($response_omie); die;
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200){
             ...
             foreach(ArrayHelper::getValue($response_omie, "body.clientes_cadastro") as $i => $cliente){
		//print_r($cliente); die;
            ...
            $body = [
                ...
                "param" => [
                    ...
                    "razao_social"              => str_replace(" "," ",substr(ArrayHelper::getValue($response_order, 'body.buyer.first_name')." ".ArrayHelper::getValue($response_order, 'body.buyer.last_name'), 0, 59)),
                    //"cnpj_cpf"                  => substr(ArrayHelper::getValue($response_order, 'body.buyer.billing_info.doc_number'),0,20),
                    "cnpj_cpf"                  => substr(ArrayHelper::getValue($billing_info,"body.billing_info.doc_number"),0,20),
                    "nome_fantasia"             => str_replace(" "," ",substr(ArrayHelper::getValue($response_order, 'body.buyer.first_name')." ".ArrayHelper::getValue($response_order, 'body.buyer.last_name'),0,100)),
                    ...
                    "contato"                   => str_replace(" "," ",substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.receiver_name'),0,100)),
                    "endereco"                  => str_replace(" "," ",substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.street_name'),0,60)),
                    "endereco_numero"           => substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.street_number'),0,10),
                    "bairro"                    => str_replace(" "," ",substr(((ArrayHelper::getValue($envio_dados, 'body.receiver_address.neighborhood.name')=="") ? "Centro" : ArrayHelper::getValue($envio_dados, 'body.receiver_address.neighborhood.name')),0,29)),
                    "complemento"               => substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.comment'),0,39),
                    "estado"                    => str_replace(" "," ",substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.state.id'),-2)),
                    "cidade"                    => str_replace(" "," ",substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.city.name'),0,40)),
	...
	//print_r($body);
            ...
            if (ArrayHelper::getValue($response_omie, 'httpCode') == 200){
                $status_retorno .= "\nCliente Criado";
                $cliente_codigo_integracao = substr(ArrayHelper::getValue($response_order, 'body.buyer.id'),0,20);
            }else{
		...
		return $status_retorno .= ($e_html)? "<br><span style='color: red;'>Produto não encontrado pelo PA no Omie principal(".$codigo_pa.")</span>": "Produto não encontrado no Omie (".$codigo_pa.")";
            }
	    $codigo_produto_omie = $response["body"]["codigo_produto"];
            ...
            $body = [
			    ...
			    "codigo_produto" 		=> $codigo_produto_omie,
                            "cfop"                      => $cfop,
    ...
    public static function criarAlterarPedidoMercadoLivre($order, $billing_info, $e_html){
        ...
        if($pedido_mercado_livre){
            ...
            $pedido_mercado_livre->buyer_doc_type       = (string) (isset($billing_info["body"]->billing_info->doc_type)) ? ArrayHelper::getValue($billing_info,"body.billing_info.doc_type") : "";
            $pedido_mercado_livre->buyer_doc_number     = (string) (isset($billing_info["body"]->billing_info->doc_number)) ? ArrayHelper::getValue($billing_info,"body.billing_info.doc_number") : "";
            ...
            $pedido_mercado_livre->buyer_doc_type       = (string) ArrayHelper::getValue($billing_info,"body.billing_info.doc_type");
            $pedido_mercado_livre->buyer_doc_number     = (string) ArrayHelper::getValue($billing_info,"body.billing_info.doc_number");
            $pedido_mercado_livre->user_id              = (string) ArrayHelper::getValue($order, 'body.seller.id');
            $pedido_mercado_livre->pack_id              = (string) ArrayHelper::getValue($order, 'body.pack_id');
