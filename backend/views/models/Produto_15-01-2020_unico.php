...
//        echo Url::to(['/p/' . $this->id . '/' . $this->slug,], $absolute);
     ...
     * @since 21/08/2017
     * @return m as imagens no formato correto para o ML (Ainda Retorna o array de uma só)
     */
    public function getUrlImagesML()
    {
        ...
        if (!empty($imagens)) {
            foreach ($imagens as $k => $v) {
                ...
                //$src[] = ['source' => 'https://www.pecaagora.com/site/imag?pid=' . $this->id]; //. '&ordem=' . $v->ordem];
            }
        } else {
    ...
    public function getUrlImage($logo = true)
    {
        ...
        if (!empty($imagens)) {
            if ($logo) {
                ...
                $src = Url::base(true) . '/site/get-link-sem-logo?produto_id=' . $this->id . '&ordem=1';
            }
        } else {
    ...
    public function getUrlImages($logo = true)
    {
        ...
        //echo "<pre>";var_dump($imagens); echo "</pre>"; die;
