...
<div class="pedido-compra-produto-filial-index">
    ...
    foreach ($modelPedidoCompraProdutoFilial as $produto) {
        ...
        $mk = $produto->valor_markup ?: number_format($produto->valor_venda / $produto->valor_compra, 2, '.', '');
        ...
        $dadosEmail1 .= str_replace(
            ["{descricao}", "{quantidade}", "{valor}", "{codpa}", "{ncm}", "{codfabri}", "{observacao}", "{mk}"],
            [$nomeProduto, $quantidade, $valor_compra, $codpa, $ncm, $codfabri, $observacao, $mk],
            $produtos
        );
    }
