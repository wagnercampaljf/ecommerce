...
namespace backend\functions\omie;
...
class SincronizarEstoqueAutomaticoSP3 extends Action
{
    public function run($parametros, $file_planilha)
    {
        ...
        $movimentacao_estoque_mestre_sp2_sp3 = new MovimentacaoEstoqueMestre;
        $movimentacao_estoque_mestre_sp2_sp3->descricao             = "Movimentação de Estoque (Sincronização de estoque - SP2 => SP3) - ".date("Y-m-d H-i-s");
        $movimentacao_estoque_mestre_sp2_sp3->salvo_em              = date("Y-m-d H:i:s");
        //$movimentacao_estoque_mestre->salvo_por             = Yii::$app->user->identity->id;
        $movimentacao_estoque_mestre_sp2_sp3->e_autorizado          = true;
        $movimentacao_estoque_mestre_sp2_sp3->filial_origem_id      = 96;
        $movimentacao_estoque_mestre_sp2_sp3->filial_destino_id     = 95;
        $movimentacao_estoque_mestre_sp2_sp3->codigo_remessa_omie   = null;
        $movimentacao_estoque_mestre_sp2_sp3->save();
        
        $movimentacao_estoque_mestre_sp3_sp2 = new MovimentacaoEstoqueMestre;
        $movimentacao_estoque_mestre_sp3_sp2->descricao             = "Movimentação de Estoque (Sincronização de estoque - SP3 => SP2) - ".date("Y-m-d H-i-s");
        $movimentacao_estoque_mestre_sp3_sp2->salvo_em              = date("Y-m-d H:i:s");
        //$movimentacao_estoque_mestre->salvo_por             = Yii::$app->user->identity->id;
        $movimentacao_estoque_mestre_sp3_sp2->e_autorizado          = true;
        $movimentacao_estoque_mestre_sp3_sp2->filial_origem_id      = 95;
        $movimentacao_estoque_mestre_sp3_sp2->filial_destino_id     = 96;
        $movimentacao_estoque_mestre_sp3_sp2->codigo_remessa_omie   = null;
        $movimentacao_estoque_mestre_sp3_sp2->save();
        
        $valor_total_dividir_remessa = 0;
        ...
        for($i=1;$i<=$total_de_paginas_posicao_estoque;$i++){
            ...
            foreach($response_omie_posicao_estoque["body"]["produtos"] as $k => $produto_estoque){
                ...
                if($estoque < 0){
                    ...
                    echo " ( Valor Total Dividir Remessa: ".$valor_total_dividir_remessa.")";
                    if($valor_total_dividir_remessa > 1500){
                        continue;
                    }
                    ...
                    if(array_key_exists($produto_estoque["cCodigo"], $produtos_dados)){
                        ...
                        $icms_n_b_c     = 0;
                        if($icms_nAliq > 0){
                            $icms_n_valor   = $valor_total_final;
                            $icms_n_b_c     = $valor_total_final*($icms_nAliq/100);
                        }
                        
                        $valor_remessa += $quantidade *$valor_unitario;
                        echo "(( Valor acumulado remessa: ".$valor_remessa."))";
...
"nBC":'.$icms_n_valor.',"nRedBC":0,"nValor":'.$icms_n_b_c.'},"ICMS_ST":{"cModBCST":"4","nAliqOP":0,"nAliqST":0,"nBCST":0,"nMargVrAd":0,"nRedBCST":0,"nValorST":0},
...
"nDesconto":0,"nQtde":'.$quantidade.',"nValUnit":'.$valor_unitario.',"rastreabilidade":[]},';
                        ...
                        $movimentacao_estoque_detalhe->movimentacao_estoque_mestre_id   = $movimentacao_estoque_mestre_sp2_sp3->id;
                        $movimentacao_estoque_detalhe->descricao                        = $codigo." | ".$quantidade;
                        $movimentacao_estoque_detalhe->salvo_em                         = date("Y-m-d H:i:s");
                        $movimentacao_estoque_detalhe->salvo_por                        = null;
                        $movimentacao_estoque_detalhe->quantidade                       = $quantidade;
                        ...
                        $valor_total_dividir_remessa += ($quantidade*$valor_unitario);
...
"nDesconto":0,"nQtde":'.$quantidade.',"nValUnit":'.$valor_unitario.',"rastreabilidade":[]},';
                    ...
                    $movimentacao_estoque_detalhe                                   = new MovimentacaoEstoqueDetalhe;
                    $movimentacao_estoque_detalhe->movimentacao_estoque_mestre_id   = $movimentacao_estoque_mestre_sp3_sp2->id;
                    $movimentacao_estoque_detalhe->descricao                        = $codigo." | ".$quantidade;
                    $movimentacao_estoque_detalhe->salvo_em                         = date("Y-m-d H:i:s");
                    $movimentacao_estoque_detalhe->salvo_por                        = null;
                    $movimentacao_estoque_detalhe->quantidade                       = $quantidade;
...
"nNFRef": "","nNfPR": ""}]},"obs":{"cObs": ""},'.$produtos_remessa.'}]}';
        ...
        if($reposta_array->cCodStatus == 0){
            $movimentacao_estoque_mestre_sp2_sp3->codigo_remessa_omie = (string) $reposta_array->nCodRem;
            $movimentacao_estoque_mestre_sp2_sp3->save();
        }
        
        if(array_key_exists("nCodRem",$reposta_array)){
            $email_texto .= "Remessa padrão  =>  ".$reposta_array->cNumeroRemessa;
            $status_padrao      = true;
...
"nNFRef": "","nNfPR": ""}]},"obs":{"cObs": ""},'.$produtos_remessa_devolucao.'}]}';
        ...
        if($reposta_array->cCodStatus == 0){
            $movimentacao_estoque_mestre_sp3_sp2->codigo_remessa_omie = (string) $reposta_array->nCodRem;
            $movimentacao_estoque_mestre_sp3_sp2->save();
        }
        
        if(array_key_exists("nCodRem",$reposta_array)){
            $email_texto .= "\nRemessa devolução  =>  ".$reposta_array->cNumeroRemessa;
        ...
        if($status_padrao || $status_devolucao){
            $assunto = "Remessas criadas dia ".date("d/m/Y");
            $email_texto .= "\n\n\nAtte,\n\nPeça Agora";
            
            var_dump(\Yii::$app->mailer->compose()
                ->setFrom([\Yii::$app->params['supportEmail'] => \Yii::$app->name])
                ->setTo(["dev.pecaagora@gmail.com","financeiro.pecaagora@gmail.com", "nfe.pecaagora@gmail.com"])
