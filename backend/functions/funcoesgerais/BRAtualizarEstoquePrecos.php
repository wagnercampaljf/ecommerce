...
class BRAtualizarEstoquePrecos extends Action
{
    public function run($parametros, $file_planilha){
        ...
        $log = "/var/tmp/log_".date('Y-M-d_H:i:s')."_".$file_planilha;
        $log_erro = "/var/tmp/log_erro".date('Y-M-d_H:i:s')."_".$file_planilha;
        // if (file_exists($log)){
        //     unlink($log);
        // }
        ...
        foreach ($LinhasArray as $i => &$linhaArray){ //Looper que vai percorrer o array com as imformações de preços a subir
            ...
            // if((   !array_key_exists($parametros_array['coluna_codigo_fabricante'])
            //     || !array_key_exists($parametros_array['coluna_nome']) 
            //     || !array_key_exists($parametros_array['coluna_preco_compra'])
            //     || !array_key_exists($parametros_array['coluna_preco']) 
            //     || !array_key_exists($parametros_array['coluna_estoque']){
        ...
        $filial_nome='BR';
        $assunto = 'Planilha LOG Atualização de preço de estoque'.' - '.$filial_nome;

        $email_texto = 'Segue em anexo a Planilha de atualização de preço de estoque';
