...

{
	...
	//print_r($file_planilha); echo "\n\n"; print_r($parametros);die;
        ...
        //var_dump($LinhasArray); die;

        $log = "/var/tmp/log_".date("Y-m-d_H-i-s").$file_planilha;
        $log_erro = "/var/tmp/log_erro_".date("Y-m-d_H-i-s").$file_planilha;
    //     if (file_exists($log)){
    //         unlink($log);
    //    }
        
        $arquivo_log = fopen($log, "a");
        //var_dump($arquivo_log);
        ...
        if(!array_key_exists($parametros_array['coluna_codigo_fabricante'], $linhaArray)          
          ...
          || !array_key_exists($parametros_array['coluna_preco_compra'], $linhaArray)){
           ...
           if($i<10716){
            ...
            $codigo_fabricante  = str_replace(" ", "", $linhaArray[$parametros_array['coluna_codigo_fabricante']]) .'.M';
            ...
            if ($produto){
                ...
                //DESCONTO DE 3%
                $preco_compra = 0.03 * $preco_compra;
                ...
                if($produto_filial){
                    ...
                    if($produto_filial->quantidade == $estoque){
                        echo ' - Estoque igual';                        
                        fwrite($arquivo_log, " - Estoque igual");
                        //continue;
                    }
                    else{
                        if(!$produto_filial->e_atualizar_quantidade_planilha)
                        {
                            echo ' - Não atualiza estoque na planilha';
                            fwrite($arquivo_log,' -Não atualiza estoque na planilha');
                            //continue;
                        }
                        else{
                            ...
                            if($produto->codigo_fabricante == '086611.M' ){
                                echo " Produto com furo de estoque";
                                $produto_filial->quantidade = 0;
                            }
        ...
        if($e){
            switch ($e->getLine()) {
                case 67:
                    $retorno = ["status" => 'Erro Informações Faltando Campo: Preco Comppra '];
                    break;
                case 71:
    ...
    echo "</prev>";
    return $retorno;
    //envio de log para e-mail

    $filial_nome='Morelati';
    $assunto = 'Planilha de atualização de preço e estoque'.' - '.$filial_nome;
