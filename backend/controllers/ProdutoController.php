...
class ProdutoController extends Controller
{
    ...
    public function actionPesquisar($termo = '')
    {
        ...
        if (!empty($termo)) {
            ...
            $query = (new Query())
                ->select([
                    ...
                    'coalesce((select quantidade from produto_filial where produto_id = p.id and filial_id = 94 limit 1), 0) as quantidade_mg1',
                    'coalesce((select quantidade from produto_filial where produto_id = p.id and filial_id = 95 limit 1), 0) as quantidade_spfilial3',
                    'coalesce((select quantidade from produto_filial where produto_id = p.id and filial_id = 96 limit 1), 0) as quantidade_fisico',
                    'coalesce((select quantidade from produto_filial where produto_id = p.id and filial_id = 93 limit 1), 0) as quantidade_mg4',
		    'coalesce((select quantidade from produto_filial where produto_id = p.id and filial_id = 96 limit 1), 0) - coalesce((select quantidade from produto_filial where produto_id = p.id and filial_id = 93 limit 1), 0) - coalesce((select quantidade from produto_filial where produto_id = p.id and filial_id = 95 limit 1), 0) as quantidade_sp2',
		...
		->orWhere(['like', 'p.codigo_fornecedor', $termo])
                ->groupBy(['p.id', 'mp.nome'])
                ->orderBy([	"quantidade_fisico" 	=> SORT_DESC, 
				"quantidade_sp2" 	=> SORT_DESC, 
				"quantidade_spfilial3" 	=> SORT_DESC, 
				"quantidade_mg4" 	=> SORT_DESC, 
				"quantidade_mg1" 	=> SORT_DESC,
				"nome"			=> SORT_ASC,
				"codigo_global"		=> SORT_ASC
			 ]);
