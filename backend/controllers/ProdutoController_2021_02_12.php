...
class ProdutoController extends Controller
{
    ...
    public function actionPesquisar($termo = '')
    {
        ...
        if (!empty($termo)) {
            ...
            $query = (new Query())
                ->select([
                    'p.id', 'p.nome', 'p.codigo_global', 'p.codigo_fabricante', 'mp.nome as marca',
                    '(select pmlppf.valor from 
                            produto_filial pf
                            inner join pedido_mercado_livre_produto_produto_filial pmlppf on pmlppf.produto_filial_id = pf.id
                            where pf.produto_id = p.id
                            order by pmlppf.id desc
                            limit 1) as valor_ml'
                ])
                ->from('produto p')
