...
class ValorProdutoFilialController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            $ml = FunctionsML::atualizarPreco($model->valor, $produto_filial);
            ...
            return $this->redirect(['view', 'id' => $model->id, 'mercadoLivre' => $ml]);
        } else {
        ...
        if (!is_null($q)) {
            $results = ProdutoFilial::find()
                ...
                ->where("codigo_global not like 'CX.%' 
                            and filial.id <> 98
                            and (produto.codigo_global like '%$q%'
    ...
    public function actionGetProdutoFilial($q, $id = null)
    {
        ...
        if (!is_null($q)) {
            $results = ProdutoFilial::find()
                // ->select(['produto_filial.id', "(coalesce((filial.nome),'') ||' - '|| coalesce((produto.nome),'') || '('|| coalesce((produto.codigo_global),'')||')' || '('|| coalesce((produto.codigo_fabricante),'')||')') as text"])
                // ->joinWith(['produto', 'filial'])
                ...
                ->select([
                    'produto_filial.id',
                    "coalesce(produto.nome, '') || ' - ' || coalesce(produto.codigo_global, '') || '(' || coalesce(filial.nome, '') || ')' || '(Est: ' || coalesce(produto_filial.quantidade::varchar , '') || ')' || '(' || coalesce(produto.codigo_fabricante, '') || ')' || '(' || coalesce((select valor_compra from valor_produto_filial where valor_produto_filial.produto_filial_id = produto_filial.id order by dt_inicio desc limit 1)::varchar, '') || ')' as text",
                    //2021-04-26"coalesce(filial.nome, '') || ' - ' || coalesce(produto.nome, '') || '(' || coalesce(produto.codigo_global, '')|| ')' || '(' || coalesce(produto.codigo_fabricante, '') || ')' || ' (' || coalesce((select valor_compra from valor_produto_filial where valor_produto_filial.produto_filial_id = produto_filial.id order by dt_inicio desc limit 1)::text, '')|| ')' as text",
                    //"coalesce(filial.nome, '') || ' - ' || coalesce(produto.nome, '') || '(' || coalesce(produto.codigo_global, '')|| ')' || '(' || coalesce(produto.codigo_fabricante, '') || ')' as text",
                ])
                ->joinWith(['produto', 'filial'])
                //->join('left JOIN', 'valor_produto_filial', 'valor_produto_filial.produto_filial_id =produto_filial.id')
                ->where([
                    'like',
                    'lower(produto.nome)',
                    strtolower($q)
    ...
    public function actionCriarml($id)
    {
        ...
        $categoria_meli_id              = "";
        $nome = str_replace(" ","%20",$model->produto->nome);
