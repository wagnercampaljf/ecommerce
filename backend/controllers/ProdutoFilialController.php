...
class ProdutoFilialController extends Controller
{
	...
	$model->quantidade = 0;
	echo "<pre>(("; var_dump($model->save()); echo "))</pre>";

	$model->meli_id = null;
	$model->quantidade = $quantidade;
	//$model->save();
	echo "<pre>(("; var_dump($model->save()); echo "))</pre>";//die;

        $resposta	= FunctionsML::CriarAnuncioML($model);
	//echo "<pre>(("; var_dump($resposta); echo "))</pre>";die;
	$status		= "Mercado Livre Principal:<br>".$resposta["permalink"];

        $produto_filial_duplicada = ProdutoFilial::find()->andWhere(["=", "produto_filial_origem_id", $model->id])->all();
        foreach ($produto_filial_duplicada as $duplicada) {
		$duplicada->meli_id = null;
	        $duplicada->save();
		$resposta = FunctionsML::CriarAnuncioML($duplicada);
		$status         = $status."<br><br>Mercado Livre Duplicada:<br>".$resposta["permalink"];
        }
	...
	$ipi = 0;
        if(!is_null($produto_filial->produto->ipi)){
                $ipi = ($produto_filial->produto->ipi/100)*$valor_retorno;
        }

        $valor_retorno = number_format(($valor_retorno+$ipi), 2, '.', '');
