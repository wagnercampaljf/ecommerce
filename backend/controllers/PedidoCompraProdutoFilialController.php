...
class PedidoCompraProdutoFilialController extends Controller
{
    ...
    public function actionCreate($id)
    {
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelProduto->load(Yii::$app->request->post())) {
                $modelProduto->valor_markup = $modelProduto->valor_venda / $modelProduto->valor_compra;

                if ($modelCompra->valor_total_pedido !== null) {
                    $modelCompra->valor_total_pedido = $modelCompra->valor_total_pedido + ($modelProduto->valor_compra * $modelProduto->quantidade);
                } else {
     ...
     * @return mixed
     */
    public function actionUpdate($id, $idProduto = null)
    {
        $modelCompra = PedidoCompra::findOne($id);
        $modelProduto = new PedidoCompraProdutoFilial();
        $searchModel = new PedidoCompraProdutoFilialSearch();
