    ...
    public function actionMercadoLivreAutorizar($id)
    {
        ...
        foreach($pedido_mercado_livre_produtos as $k => $pedido_mercado_livre_produto){
            ...
            foreach($pedido_mercado_livre_produto_produto_filials as $i => $pedido_mercado_livre_produto_produto_filial){
                ...
                $produtos_email[$produto_filial->filial->id][] = [
                    'codigo' => $produto_filial->produto->codigo_fabricante,
                    'descricao' => $produto_filial->produto->nome." (".$produto_filial->produto->codigo_global.")",
                    'quantidade' => $pedido_mercado_livre_produto_produto_filial->quantidade,
                    'valor' => $pedido_mercado_livre_produto_produto_filial->valor,
        ...
        foreach ($produtos_email as $filial_id => $produto_email){
            
            $filial = Filial::find()->andWhere(['=','id', $filial_id])->one();
            
            $email_texto = $model->email_texto;
            
            $emails = $model->email_enderecos.(($filial->email_pedido != null && $filial->email_pedido != "") ? ",".$filial->email_pedido : "");
            $emails = str_replace(";",",",str_replace(" ","",$emails));
            $emails_destinatarios = explode(",",$emails);
            //$emails_destinatarios[] = $filial->email_pedido;
            ...
            foreach($produto_email as $produto_email_filial){
                
                //echo "<pre>"; print_r($produto_email_filial); echo "</pre>";
                
                if($quantidade_produtos <= 0){
                    $email_texto = str_replace("{codigo}",$produto_email_filial['codigo'], $email_texto);
                    $email_texto = str_replace("{descricao}",$produto_email_filial['descricao'], $email_texto);
                    $email_texto = str_replace("{quantidade}",$produto_email_filial['quantidade'], $email_texto);
                    $email_texto = str_replace("{valor}",$produto_email_filial['valor'], $email_texto);
                    
                    if($produto_email_filial['quantidade'] != null && $produto_email_filial['quantidade'] != ""){
                        $email_texto = str_replace("{observacao}",$produto_email_filial['observacao'], $email_texto);
                    } 
                    else{
                        $email_texto = str_replace("\nObservação: {observacao}","", $email_texto);
                    }
                    
                    $assunto .= "Pedido ".$produto_email_filial['codigo'];
                    $quantidade_produtos++;
                }
                else{
                    $texto = "\n\nCódigo: ".$produto_email_filial['codigo']."\nDescrição: ".$produto_email_filial['descricao']."\nQuantidade: ".$produto_email_filial['quantidade']."\nValor: ".$produto_email_filial['valor'].(($produto_email_filial['observacao'] != null && $produto_email_filial['observacao'] != "") ? "\nObservação: ".$produto_email_filial['observacao'] : "")."\n\n\nEnvio";
                    $email_texto = str_replace("\nEnvio:",$texto, $email_texto);
                    
                    $assunto .= " - ".$produto_email_filial['codigo'];
                }
            }
