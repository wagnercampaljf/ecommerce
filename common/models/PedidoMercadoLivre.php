 ...
 * @property integer $porcentagem_pedido
...
class PedidoMercadoLivre extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['filial_id', 'transportadora_id', 'porcentagem_pedido'], 'integer'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'filial_id'    => 'Filial ID',
            'porcentagem_pedido'    => 'Porcentagem',
        ...
        foreach ($contas_ml as $conta_ml) {
            ...
            if ($response_order["httpCode"] < 300) {
                ...
                if ($envio_dados['httpCode'] >= 300) {
                    ...
                    $pedido_ml = PedidoMercadoLivre::findOne(['pedido_meli_id' => $order]);

                    if ($pedido_ml->filial_id == null) {
    ...
    public static function criarPedidoOmie($user_id, $response_order, $envio_dados, $billing_info, $e_html, $order)
    {
        ...
        $pedido_ml = PedidoMercadoLivre::findOne(['pedido_meli_id' => $order]);
        $filial_id = $pedido_ml->filial_id;
        $transportadora_id = $pedido_ml->transportadora_id;
        $transportadora = Transportadora::findOne($transportadora_id);
        $perc_pedido = $pedido_ml->porcentagem_pedido !== null ? $pedido_ml->porcentagem_pedido / 100 : 1;
        ...
        $bodyCPF = [
            ...
            "param" => [
                ...
                "clientesFiltro" => [
                    "cnpj_cpf" => $pedido_ml->buyer_doc_number,
                ]
            ]
        ];

        $response_omie = $omie->consulta_cliente("api/v1/geral/clientes/?JSON=", $bodyCPF);
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200) {
            ...
            $response_omie = $omie->consulta_cliente("api/v1/geral/clientes/?JSON=", $bodyIntegracao);
            ...
            if (ArrayHelper::getValue($response_omie, 'httpCode') == 200) {
                ...
                $response_omie = $omie->consulta_cliente("api/v1/geral/clientes/?JSON=", $bodyAlteração);
                ...
                if ($pedido_ml->filial_id) {

                    $cep = substr(str_replace('-', '', $pedido_ml->receiver_zip_code), 0, 8);
                    ...
                    $body = [
                        ...
                        "param" => [
                            ...
                            "telefone1_ddd"             => substr($pedido_ml->receiver_phone, 0, 2),
                            "telefone1_numero"          => substr($pedido_ml->receiver_phone, 2, 9),
                            "contato"                   => str_replace(" ", "%20", substr($pedido_ml->receiver_name, 0, 100)),
                            "endereco"                  => str_replace(" ", "%20", substr($pedido_ml->receiver_street_name, 0, 59)),
                            "endereco_numero"           => substr($pedido_ml->receiver_street_number, 0, 10),
                            "bairro"                    => utf8_encode(str_replace(" ", "%20", substr($pedido_ml->receiver_neighborhood_name == "" ? "Centro" : $pedido_ml->receiver_neighborhood_name, 0, 30))),
                            "complemento"               => utf8_encode(substr($pedido_ml->receiver_comment, 0, 39)),
                            "estado"                    => strtoupper(str_replace(" ", "%20", $pedido_ml->receiver_state_id)),
                            "cidade"                    => $endereco->ibge, //"3536703",//$endereco->ibge,
                            "cep"                       => substr($pedido_ml->receiver_zip_code, 0, 10),
                    ...
                    $body = [
                        ...
                        "param" => [
                            ...
                            "estado"                    => strtoupper(str_replace(" ", "%20", substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.state.id'), -2))),
                            "cidade"                    => $endereco->ibge, //"3501509",//$endereco->ibge,
                            "cep"                       => substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.zip_code'), 0, 10),
                            "email"                     => "cliente.pecaagora@gmail.com", //ArrayHelper::getValue($response_order, 'body.buyer.email'),
                        ]
                    ];
        ...
        $body = [
            ...
            "app_secret" => $app_secret,
            "param" => [
                "codigo_pedido_integracao" => $order,
            ]
        ];
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200) {
            ...
            $pedido_ml->codigo_pedido_omie = ArrayHelper::getValue($response_omie, 'body.pedido_venda_produto.cabecalho.codigo_pedido');
            if ($pedido_ml->save(false)) {
                $status_retorno .= "\nCodigo Omie adicionado no pedido do ML";
            } else {
                $status_retorno .= "\nCodigo Omie não adicionado no pedido do ML";
            }
        } else {
            $pedido_ml->codigo_pedido_omie = null;
            $pedido_ml->save(false);
        }
        ...
        $aliquota_uf_destino = [
            ...
            'GO' => 0.17, 'MA' => 0.18, 'MS' => 0.17, 'MT' => 0.17, 'MG' => 0.18, 'SP' => 0.18, 'RJ' => 0.18, 'SC' => 0.17,
            'TO' => 0.18, 'SE' => 0.18, 'RR' => 0.17, 'RO' => 0.175, 'RS' => 0.175, 'RN' => 0.18, 'PI' => 0.17, 'PR' => 0.18,
            'PB' => 0.18, 'PE' => 0.18, 'PA' => 0.17
        ];


        $estado = $pedido_ml->filial_id <> null ? $pedido_ml->receiver_state_id : substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.state.id'), -2);
        $estado = strtoupper(substr($estado, -2));
        $perc_fcp = 0;

        if ($estado == "AL" || $estado == "PI") {
            $perc_fcp = 0.01;
        }
        ...
        if ($user_id == '193724256' || $user_id == '435343067') {
            if ($estado !== "SP") {
                ...
                if (in_array($estado, $aliquota_interestadual_sp)) {
                    $perc_aliquota = 0.12;
                } else {
                    $perc_aliquota = 0.07;
                }
            ...
            if ($estado !== "MG") {
                ...
                if (in_array($estado, $aliquota_interestadual_mg)) {
                    $perc_aliquota = 0.12;
                } else {
                    $perc_aliquota = 0.07;
                }

                if (strlen(substr(str_replace(" ", "", $pedido_ml->buyer_doc_number), 0, 20)) > 11) {
                    if ($inscricao_estadual !== '') {
                        $cfop = '6102';
                    } else {
                        $cfop = '6108';
        ...
        if ($pedido_ml->codigo_pedido_omie <> null) {

            $produto_ml = PedidoMercadoLivreProduto::findOne(['pedido_mercado_livre_id' => $pedido_ml->id]);
            ...
            if ($produto_cotacao_ml) {
                ...
                foreach ($produto_cotacao_ml as $cotacao) {
                    ...
                    $frete          = $pedido_ml->filial_id <> null ? $pedido_ml->shipping_option_list_cost : ArrayHelper::getValue($envio_dados, 'body.shipping_option.cost') - ArrayHelper::getValue($envio_dados, 'body.shipping_option.list_cost');
                    $frete          = ($frete < 0) ? $frete * (-1) : $frete;
                    $total_frete    = $frete;
                    $valor_unitario = $valor_unitario = $pedido_ml->filial_id <> null ? ($valor_unitario - ($tarifas / $quantidade)) : $valor_unitario - (($tarifas / $quantidade) + ($frete / $quantidade));
                    ...
                    $bc_icms = 0;
                    $valor_icms = 0;
                    $difal = $aliquota_uf_destino[$estado] - $perc_aliquota;

                    if ($user_id == '193724256' || $user_id == '435343067') {
                        if ($estado !== "SP") {
                            $bc_icms = ($valor_unitario * $quantidade) * $perc_pedido;
                            $valor_icms = $bc_icms * $perc_aliquota;
                        }
                    } else {
                        $bc_icms = (($valor_unitario * $quantidade) + $tarifas + $frete) * $perc_pedido;
                        $valor_icms = $bc_icms * $perc_aliquota;
                    }
                    ...
                    if ($cfop == '6108') {
                        ...
                        $aliq_interna_uf_destino = $aliquota_uf_destino[$estado] * 100;
                    ...
                    if ($bc_icms > 0) {
                        $base_cofins = (($valor_unitario * $quantidade) + $tarifas + $frete - $valor_icms) * $perc_pedido;
                        $base_pis    = (($valor_unitario * $quantidade) + $tarifas + $frete - $valor_icms) * $perc_pedido;
                    } else {
                        $base_cofins = (($valor_unitario * $quantidade) + $tarifas + $frete) * $perc_pedido;
                        $base_pis    = (($valor_unitario * $quantidade) + $tarifas + $frete) * $perc_pedido;
                    }
                    ...
                    array_push($det, [
                        ...
                        "produto" => [
                            ...
                            "valor_unitario"            => $valor_unitario * $perc_pedido, //ArrayHelper::getValue($response_order, 'body.order_items.0.unit_price'),
                        ],
                    ]);
                }
            }
            ...
            if ($bc_icms > 0) {
                $base_cofins = ($valor_unitario * $quantidade) + $tarifas + $frete - $valor_icms;
                $base_pis = ($valor_unitario * $quantidade) + $tarifas + $frete - $valor_icms;
            } else {
                $base_cofins = ($valor_unitario * $quantidade) + $tarifas + $frete;
                $base_pis = ($valor_unitario * $quantidade) + $tarifas + $frete;
            }
            ...
            "param" => [
                [
                    ...
                    "frete" => [
                        ...
                        "outras_despesas"       => $total_tarifa * $perc_pedido,
                        "valor_frete"           => $total_frete * $perc_pedido,
                    ],
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200) {
            ...
            if ($pedido_ml->codigo_pedido_omie) {
                $status_retorno .= "\nPedido Alterado";
            } else {
                $status_retorno .= "\nPedido criado";
            }

            $pedido_ml->codigo_pedido_omie = ArrayHelper::getValue($response_omie, 'body.codigo_pedido');
    ...
    {
        $modelPisCofins = PisCofins::findOne(['ncm' => $produto->codigo_montadora]);

        if (!empty($modelPisCofins)) {
            return $modelPisCofins->pis_cofins;
        } else {
    ...
    public static function criarAlterarPedidoMercadoLivre($order, $billing_info, $e_html)
    {
        ...
        $pedido_mercado_livre = PedidoMercadoLivre::findOne(['pedido_meli_id' => ArrayHelper::getValue($order, 'body.id')]);
        ...
        if ($pedido_mercado_livre) {
            ...
            if ($pedido_mercado_livre->buyer_doc_number == null || $pedido_mercado_livre->buyer_doc_number == '') {
            ...
            $pedido_mercado_livre->pack_id              = (string) ArrayHelper::getValue($order, 'body.pack_id');
            $pedido_mercado_livre->email_enderecos      = "notafiscal.pecaagora@gmail.com, compras.pecaagora@gmail.com";
            $pedido_mercado_livre->porcentagem_pedido   = 100;
