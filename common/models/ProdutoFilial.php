 ...
 * @property boolean $e_atualizar_quantidade_planilha
 * @property boolean $e_atualizar_preco_planilha
 *
...
class ProdutoFilial extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['produto_id', 'filial_id', 'quantidade', 'envio', 'estoque_minimo'], 'integer'],
            [['meli_id', 'meli_id_sem_juros', 'meli_id_full', 'status_b2w', 'atualizar_preco_mercado_livre', 'e_atualizar_quantidade_planilha', 'e_atualizar_preco_planilha'], 'safe'],
    ...
    public function attributeLabels()
    {
	    ...
	    'e_atualizar_quantidade_planilha' => 'Atulizar estoque pela planilha?',
	    'e_atualizar_preco_planilha' => 'Atulizar preço pela planilha?',
        ];
    }
    ...
    {
        //print_r($this->valorMaisRecente->valor); die;
    ...
    {
        if (is_null($this->meli_id) && is_null($this->meli_id_sem_juros) && is_null($this->meli_id_flex)) {
        ...
        if (!is_null($this->meli_id_sem_juros) && $this->meli_id_sem_juros <> "") {
            ...
            $meli_id_texto              .= "'" . $this->meli_id_sem_juros . "',";
        ...
        if (!is_null($this->meli_id_full) && $this->meli_id_full <> "") {
            ...
            $meli_id_texto              .= "'" . $this->meli_id_full . "',";
            $produtos_mercado_livre[]     = ["token" => $meliAccessToken, "meli_id" => $this->meli_id_full, "tipo" => "meli_id_full"];
        }
        if (!is_null($this->meli_id_flex) && $this->meli_id_flex <> "") {
            $meli_id_array[]             = $this->meli_id_flex;
            $meli_id_texto              .= "'" . $this->meli_id_flex . "',";
            $produtos_mercado_livre[]     = ["token" => $meliAccessToken, "meli_id" => $this->meli_id_flex, "tipo" => "meli_id_flex"];
        }
        ...
        foreach ($produtos_filial_duplicado as $produto_filial_duplicado) {
            ...
            if (!is_null($produto_filial_duplicado->meli_id_flex) && $produto_filial_duplicado->meli_id_flex <> "") {
                ...
                $produtos_mercado_livre[] = ["token" => $meliAccessToken_conta_duplicada, "meli_id" => $produto_filial_duplicado->meli_id_flex, "tipo" => "meli_id_flex"];
            }
        }

        $meli_id_texto = trim($meli_id_texto, ",");
        print_r($meli_id_texto);
		...
		echo "(((((FLEX)))))";
            ...
            //if($this->filial_id == 94 || $this->filial_id == 99){
            //$valor_final = $valor_final * 1.15;
            //}
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        FunctionsML::atualizarQuantidade($this->produto);
