 ...
 * @property string $localizacao_mg
...
class Produto extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['descricao', 'localizacao', 'localizacao_mg', 'codigo_similar', 'aplicacao_complementar', 'aplicacao', 'video', 'codigo_barras', 'cest'], 'string'],
            [['peso', 'altura', 'largura', 'profundidade', 'ipi', 'valor', 'valor_cnpj', 'valor_compra', 'st'], 'number'],
            ...
            [['anoModelo_id', 'e_usado', 'e_medidas_conferidas', 'e_ativo', 'e_localizacao_bloqueado', 'e_valor_bloqueado','st'], 'safe'],
    ...
    public function getUrlImageML()
    {
        ...
        // 
        //	}
        //Alteração dia 18/10/2021
        if (!empty($imagens)) {
            $src = '/imagens/produto_' . $this->id . '/' . $this->id . '_' . $imagem->ordem . '.webp';
        } else {
            $src = "https://www.pecaagora.com/frontend/web/assets/img/produtos/no-image.png";
        }
    ...
    public function getUrlImagesML()
    {
        ...
        ArrayHelper::multisort($imagens, ['ordem'], [SORT_ASC]);
         ...
         if (!empty($imagens)) {
             ...
             foreach ($imagens as $k => $v) {
                 //if (file_exists("/var/www/html/frontend/web/assets/img/imagens_temporarias/" . $v->id . ".jpg")) {
                     //$src[] = ['source' => 'https://www.pecaagora.com/frontend/web/assets/img/imagens_temporarias/' . $v->id . '.jpg'];
                 if (file_exists("/var/www/html/frontend/web/assets/img/imagens_temporarias/" . $v->produto_id . '_' . $v->ordem . ".jpg")) {
                     $src[] = ['source' => 'https://www.pecaagora.com/frontend/web/assets/img/imagens_temporarias/' . $v->produto_id . '_' . $v->ordem . '.jpg'];
                     echo " - Imagem já existente";
                     
                 } else {

                     //$caminho    = '/var/www/imagens_produto/produto_' . $v->produto_id . '/' . $v->produto_id . '_' . $v->ordem . '.webp';
                     if(file_exists('/var/www/imagens_produto/produto_' . $v->produto_id . '/' . $v->produto_id . '_' . $v->ordem . '.webp')){
                        $caminho    = '/var/www/imagens_produto/produto_' . $v->produto_id . '/' . $v->produto_id . '_' . $v->ordem . '.webp';
                        copy($caminho, '/var/www/html/frontend/web/assets/img/imagens_temporarias/' . $v->produto_id . '_' . $v->ordem . ".webp", stream_context_create($stream_opts));
                        $resultado = shell_exec('cd /var/www/html/frontend/web/assets/img/imagens_temporarias/ ; mogrify -format jpg ' . $v->produto_id . '_' . $v->ordem . '.webp');
                        $src[] = ['source' => 'https://www.pecaagora.com/frontend/web/assets/img/imagens_temporarias/' . $v->produto_id . '_' . $v->ordem . '.jpg'];
                        echo " - Imagem criada";
                     }
                     else{
        ...
        $src[] = ['source' => 'https://www.pecaagora.com/frontend/web/assets/img/imagens_temporarias/pecaagoraglobal.jpg'];
    ...
    public function getUrlImagesMLSemLogo()
    {
        ...
        if (!empty($imagens)) {
            foreach ($imagens as $k => $v) {
                ...
                $caminho    = '/var/www/imagens_produto/produto_' . $v->produto_id . '/' . $v->produto_id . '_' . $v->ordem . '_sem_logo.webp';
                copy($caminho, '/var/www/html/frontend/web/assets/img/imagens_temporarias/' . $v->id . ".webp", stream_context_create($stream_opts));
    ...
    public function getUrlImagesB2WWebp()
    {
        ...
        if (!empty($imagens)) {
            foreach ($imagens as $k => $imagem) {
                ...
                copy($caminho, '/var/www/html/frontend/web/assets/img/imagens_temporarias/' . $imagem->id . "_sem_logo.webp", stream_context_create($stream_opts));
    ...
    public function atualizarMercadoLivre()
    {
        ...
        //echo "<pre>"; print_r($response_conta_duplicada); echo "</pre>";
        ...
        if (is_object($response) && ArrayHelper::getValue($user, 'httpCode') < 400) {
            ...
            foreach ($produto_filiais as $k => $produto_filial) {
                ...
                $mensagem .= $this->atualizarProdutoML($meli, $meliAccessToken, $produto_filial->meli_id, $preco_conta_principal, $produto_filial->quantidade, null, "CONTA PRINCIPAL - PRINCIPAL");
                ...
                if ($produto_filial_conta_duplicada) {
                    ...
                    $response_item = $meli->get("/items/" . $produto_filial_conta_duplicada->meli_id . "/?access_token=" . $meliAccessToken_conta_duplicada);
                    if ($response_item["httpCode"] >= 300) {
                        echo " - Produto não encontrado no ML";
                    } else {
                        if (ArrayHelper::getValue($response_item, 'body.listing_type_id') == "gold_pro") {
                    ...
                    if (!is_null($produto_filial_conta_duplicada->meli_id_sem_juros) && $produto_filial_conta_duplicada->meli_id_sem_juros <> "") {
                        ...
                        $response_item = $meli->get("/items/" . $produto_filial_conta_duplicada->meli_id_sem_juros . "/?access_token=" . $meliAccessToken_conta_duplicada);
                        if ($response_item["httpCode"] >= 300) {
                            echo " - Produto não encontrado no ML";
                        } else {
                            if (ArrayHelper::getValue($response_item, 'body.listing_type_id') == "gold_pro") {
    ...
    public function atualizarMLDiasExpedicao()
    {
        ...
        if ($produtos_filiais->produto->dias_expedicao <= 0 || $produtos_filiais->produto->dias_expedicao == null) {
            $retorno[$produto_filial->id]["meli_id_status"] = "Nome não alterado";
            return $retorno;
        }
        $dias = $produtos_filiais->produto->dias_expedicao;
        ...
        foreach ($produtos_filiais as $j => $produto_filial) {
            ...
            if (!is_null($produto_filial->meli_id) && $produto_filial->meli_id <> "") {
                ...
                if ($response['httpCode'] >= 300) {
                    ...
                    echo "\nDias de Expedição (MELI_ID) - " . $this->nome;
                }
            }
            if (!is_null($produto_filial->meli_id_sem_juros) && $produto_filial->meli_id_sem_juros <> "") {
                $retorno[$produto_filial->id]["meli_id_sem_juros"] = $produto_filial->meli_id_sem_juros;
                ...
                if ($response['httpCode'] >= 300) {
                    ...
                    echo "\nDias de Expedição (MELI_ID_SEM_JUROS) - " . $this->nome;
                }
            }
            if (!is_null($produto_filial->meli_id_full) && $produto_filial->meli_id_full <> "") {
                $retorno[$produto_filial->id]["meli_id_full"] = $produto_filial->meli_id_full;
            ...
            if (!is_null($produto_filial->meli_id_flex) && $produto_filial->meli_id_flex <> "") {
                ...
                if ($response['httpCode'] >= 300) {
                    ...
                    echo "\nDias de Expedição alterado (MELI_ID_FLEX) - " . $this->nome;
                }
            }
        }
     ...
     public function atualizarMLVideo()
     {
        ...
        foreach ($produtos_filiais as $j => $produto_filial) {            
            ...
            // var_dump($produto_filial->produto->nome);
            // die; 
            ...
            // $valor_produto_filial = ValorProdutoFilial::find()    ->innerJoin($produto_filial)
            //                                                         ->andWhere('=','valor_produto_filial.produto_filial_id',$produto_filial->id)
            //                                                         ->one();                           
                                       
                        
            // foreach($valor_produtos_filiais as $valor_produto_filial){

            //  var_dump(strlen($valor_produto_filial->valor));
            //  die;   
            $body = [
    ...
    public function atualizarDaddosMLf()
    {
