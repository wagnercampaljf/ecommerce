...
echo "(222)";
    ...
    public static function criarPedidoOmie($user_id, $response_order, $envio_dados, $billing_info, $e_html, $order)
    {
        ...
        switch ($user_id) {
            ...
            case "195972862":
                ...
                $codigo_transportadora      = "2388465845";
                $codVend                    = 2388488707;
                $codigo_conta_corrente      = 2389613314;
                break;
        }
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200) {
            ...
            $body = [
                ...
                "param" => [
                    ...
                    "cidade"                    => $endereco->ibge, //"3137205",//$endereco->ibge,
                    "cep"                       => substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.zip_code'), 0, 10),
        ...
        if (ArrayHelper::getValue($response_omie, 'httpCode') == 200) {
            ...
            if ($user_id == '193724256' || $user_id == '435343067') {
                if (substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.state.id'), -2) !== "SP") {
                    ...
                    if (strlen(substr($billing_info["body"]->billing_info->doc_number, 0, 20)) > 11) {
            ...
            array_push($det, [
                ...
                "imposto" => [
                    ...
                    "icms" => [
                        ...
                        "base_icms" => ($valor_unitario + $frete),
                        "cod_sit_trib_icms" => "00",
                        "modalidade_icms" => "3",
                        "origem_icms" => "0",
                        "perc_red_base_icms" => 0,
                        "valor_icms" => ($valor_unitario + $frete) * $perc_aliquota,
                        "perc_fcp_icms" => $perc_fcp,
                        "base_fcp_icms" => ($valor_unitario + $frete),
                        "valor_fcp_icms" => ($valor_unitario + $frete) * $perc_fcp
                    ],
                    "icms_ie" => [
                        "base_icms_uf_destino" => ($valor_unitario),
                        "aliq_icms_FCP" => $perc_fcp,
                        ...
                        "valor_fcp_icms_inter" => ($valor_unitario) * $perc_fcp,
                        "valor_icms_uf_dest" => ($valor_unitario) * $aliquota_uf_destino[substr(ArrayHelper::getValue($envio_dados, 'body.receiver_address.state.id'), -2)],
                        "valor_icms_uf_remet" => ($valor_unitario) * $perc_aliquota,
                    ],
                ],
                "produto" => [
                ...
                "param" => [
                    [
                        ...
                        "informacoes_adicionais"    => [
                            ...
                            "dados_adicionais_nf"        => ($user_id == "195972862") ? "Mercadoria destinada a uso e consumo, vedado o aproveitamento do crédito nos termos do inciso III do art.70 do RICMS." : "",
                        ],
                    ],
                ]
            ];

            $response_omie = $omie->CriarPedido("api/v1/produtos/pedido/?JSON=", $body);
            if (ArrayHelper::getValue($response_omie, 'httpCode') == 200) {
                $status_retorno .= "\nPedido criado";
    ...
    public static function GerarPisCofins($produto)
    {
        ...
        $ncm_produto = str_replace('.', '', $produto->codigo_montadora);
        $pattern = '/' . $ncm_produto . '/';
        if (preg_match($pattern, $codigos)) {
