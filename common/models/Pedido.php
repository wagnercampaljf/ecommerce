 ...
 * @property bool $e_pedido_autorizado
 * @property bool $e_email_estoque
 * @property integer $compra_filial_id
 * @property integer $origem_pedido
 * @property integer $conta_corrente_id
 * @property integer $codigo_pedido_omie
 * @property bool $e_verificado
 *
...
class Pedido extends \yii\db\ActiveRecord
{
    ...
    public static $transportadorasClasses = [
        ...
        7 => '\frete\conectores\MelhorEnviosPacFrete',
        8 => '\frete\conectores\MelhorEnviosSedexFrete',
    ];
    ...
    {
        return [
            [['conta_corrente_id', 'transportadora_id', 'valor_total', 'comprador_id', 'filial_id', 'forma_pagamento_id', 'valor_frete'], 'required'],
            ...
            [['conta_corrente_id', 'comprador_id', 'compra_filial_id', 'codigo_pedido_omie', 'filial_id', 'transportadora_id', 'forma_pagamento_id', 'plp_id', 'administrador_id', 'tipo_frete', 'origem_pedido'], 'integer'],
            [['token_moip', 'token_payment'], 'string', 'max' => 255],
            [['etiqueta'], 'string', 'max' => 50],
            [['observacao'], 'string', 'max' => 600],
            [['email_enderecos'], 'string', 'max' => 400],
            [['email_texto', 'email_assunto', 'comentario'], 'string'],
            [['e_pedido_autorizado', 'e_email_estoque', 'e_verificado'], 'boolean'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'observacao' => 'Observação Pedido / Pagamento',
            'e_pedido_autorizado' => 'Autorizar Pedido',
            'e_email_estoque' => 'Email Estoque',
    ...
    {
        return $this->hasMany(
            ProdutoFilial::className(),
            ['id' => 'produto_filial_id']
        )->viaTable('pedido_produto_filial', ['pedido_id' => 'id']);
    }
    ...
    public function mudarStatus($novoStatus)
    {
        ...
        $pedido = Pedido::findOne($this->id);
        $comprador = $pedido->comprador;
        //
        \Yii::$app->mailer->compose('mudanca_status', ['nome' => $comprador->nome, 'status' => $pedido->statusAtual->tipoStatus->nome, 'id' => $pedido->id])
        ...
        if ($pedido->statusAtual->tipoStatus->id == self::PEDIDO_CONFIRMADO || $pedido->statusAtual->tipoStatus->id == self::PEDIDO_CANCELADO) {
            \Yii::$app->mailer->compose(
                'mudanca_status_lojista',
                ['nome' => $usuario->nome, 'status' => $pedido->statusAtual->tipoStatus->nome, 'id' => $pedido->id]
            )
                ->setFrom(Yii::$app->params['supportEmail'])
                ->setTo([$usuario->email, 'sac@pecaagora.com'])
                ->setCc('sac@pecaagora.com')
                ->setSubject('Mudança de status')->send();
        }

        //Se voltar a funcionalidade de envio automatico rever código abaixo
