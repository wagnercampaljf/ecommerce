...
class CallbackmlController extends Controller
{
    ...
    public function actionPedidoml(){
        ...
        $arquivo_log_nome = "/var/tmp/log_callback_ml_".date("Y-m-d_H-i-s").".csv";
	...
	$arquivo_log_nome = "/var/tmp/log_callback_ml".$nome."_".date("Y-m-d_H-i-s").".csv";
        $arquivo_log = fopen($arquivo_log_nome, "a");
        fwrite($arquivo_log, date("Y-m-d_H-i-s")."\n");
	//$texto_post = implode(";", $post_ml);
	//fwrite($arquivo_log, "\n".$texto_post."\n");
...
Skype: pecaagora";
	    ...
	    $pedido_mercado_livre->email_assunto = "Pedido {codigo_fabricante} * {quantidade} - {nome} ({margem})". ' - '. $pedido_mercado_livre->pedido_meli_id;
	    if($pedido_mercado_livre->user_id == "435343067"){
		$pedido_mercado_livre->email_assunto = "Novo Pedido {codigo_fabricante} * {quantidade} - {nome} ({margem})". ' - '. $pedido_mercado_livre->pedido_meli_id;
	    }
