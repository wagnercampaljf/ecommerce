...
$dadosProduto = (new Query())
    ...
    ->where(" quantidade > 0 and p.id = ".$produto->id." and pf.id in (select distinct produto_filial_id from valor_produto_filial) ")
    ->all();
...
<div itemscope itemtype="http://schema.org/Product" class="site-product">
    <div class="panel panel-body">
        <div class="col-sm-12 colzero">
            <span itemprop="name " style="word-wrap: break-word; font-size: 18px" class="h3"><b><?= $produto->nome . " <font color='#FFFFFF'>(" . $produto->codigo_global . ")</font>" //$produto->label 
                                                                                                ?> </b></span><br><br>
            <?php
        ...
        <div class="main-product-view">
                ...
                <div id="carousel" class="carousel slide" data-ride="carousel">
                    ...
                    <div class="carousel-inner img-container" id="zoom">
                        <?php foreach ($imagens as $k => $imagem) {
                            ...
                            //echo Html::img($imagem, $options);
                            list($width, $height, $type, $attr) = $imagem;
            ...
            <div class=" col-lg-4 col-md-2 col-sm-4 col-sx-12 text-center hidden-xs">
                <img src="<?= Url::to('@assets/'); ?>img/1D-708x178.png"  class="img-ht img-fluid rounded acende img-responsive" style="width:100%; height: 100%" alt="imagem responsiva">
            </div><br>



            <div class=" col-lg-4 col-md-2 col-sm-4 col-sx-12 text-center hidden-xs" style="background-color:  #7a1002">
                <span class="text-center wrapper" style="color: #ffffff; font-size: 15px;padding: 6px; border-bottom:solid 2px #ffffff"><img src="<?= Url::to('@assets/'); ?>img/relogio.gif" style="width: 32px; height: 32px; " border="0" alt="relogio-imagem-animada-0148">
                    <h4> &nbsp OFERTA ESPECIAL </h4>
                </span>
                <!--<img src="<?= Url::to('@assets/'); ?>img/chapeu-natal.png" style="width: 45px; height: 40px; " border="0" alt="relogio-imagem-animada-0148">--><br>
            ...
            <div class=" col-lg-4 col-md-6 col-sm-4 col-sx-12" style="background-color: #d7d7d7;"><br>
                ...
                <div class="panel panel-primary price lead text-center">
                    ...
                    <!-- PREÇO COM DESCONTO  -->
                    <!--  <span style=" font-size: 16px">De: </span> <span style="text-decoration: line-through; font-size: 16px">R$<?= (($produto->id) % 2 == 1) ?  number_format(($dadoProduto['menor_valor'] / 0.82), 2, ',', '') : number_format(($dadoProduto['menor_valor'] / 0.89), 2, ',', '') ?></span> <span style=" font-size: 16px"> por</span>-->

                    <?php
                    $ativo = true;
                    if (isset(Yii::$app->session["carrinho"][$dadoProduto['pf_id']])) {
                        $ativo = false;
                    }
                    ?>
                    <?php if (!empty($dadoProduto) && !empty($dadoProduto['menor_valor'])) { ?>
                        <div itemprop="offers" itemscope itemtype="http://schema.org/AggregateOffer" class="panel-body">
                            <!--  <small >Melhor Preço</small>-->
                            <br>
                            <span itemprop="lowPrice" class="h1 text-success"><?= str_replace(',', ',<small>', Yii::$app->formatter->asCurrency($dadoProduto['menor_valor'])) . '</small>'; ?></span>

                            <div class="row">
                                <br>

                                <a href="<?= Yii::$app->urlManager->hostInfo . Yii::$app->urlManagerFrontEnd->baseUrl . '/carrinho/adicionar-produto-carrinho/?id=' . $dadoProduto['pf_id'] ?>" aria-disabled="true" class="btn-lg col-lg-offset-2 col-lg-8  btn-danger  <?= ($ativo ?: 'disabled') ?>" style="height: 46px;"><i class="no-color fa fa-shopping-cart"></i><b> <?= ($ativo ? "Comprar" : 'Já Adicionado') ?></b>
                                </a>
                            </div>
                    ...
                    <?php } ?>
                    <small>
                        <small style="color: white">Vendido e entregue por <b><?= $dadoProduto['filial'] ?></b></small>
                    </small>

                    <br>
                    <!--<div class="panel-footer">
                            <span class="h5"><a id="outrasLojas" href="#tabconteudo">Veja nossas outras <b><?= "" //count($filiais) 
                                                                                                            ?></b> ofertas deste produto <i class="fa fa-arrow-right" aria-hidden="true"></i></a></span>
                    </div>-->
                </div>
                ...
                <!-- Divide de 60x -->
                <!-- DIVIDE DE 6 VEZES
                <div><i class="fa fa-credit-card" aria-hidden="true"></i>  em até 6x de <?= number_format(($dadoProduto['menor_valor'] / $divide), 2, ',', '') ?>   c/ juros no cartão de crédito.
                            ...
                            <div class="modal-content">
                                ...
                                <div class="tab-content" style="border: 1px solid #bfbfbf" id="myTabContent">
                                    <div class="tab-pane fade " id="home" role="tabpanel" aria-labelledby="home-tab">
                                        <div class="modal-body">
                                            ...
                                            foreach (CreditCard::$parcelamentoMOIP as $nrParcelas => $juros) {
                                                $valor_prazo = $dadoProduto['menor_valor'] + 0.69;
                                    ...
                                    <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                                        <h3 style="margin-left: 10px">R$<?= number_format(($dadoProduto['menor_valor'] + $taxafixa), 2, ',', '') ?></h3> <p style="margin-left: 10px">  O boleto será gerado após a finalização de sua compra. Imprima e pague no banco ou pague pela internet utilizando o código de barras do boleto.</p>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>
                ...
                <div class="form-inline" action="">
                    ...
                    <div class="filial_<?= $dadoProduto['filial_id'] ?> product-view-div panel-lojas-product-view-datails col-lg-12 col-md-12 col-sm-12 col-xs-12 text-left">
                ...
                <div class="collapse" id="collapseExample">
                    <div class="card card-body">
                        ...
                        <?php } ?>
                        <div class="product-detail">
                            <small style="color: #d7d7d7">Vendido e entregue por <b><?= $dadoProduto['filial'] ?></b></small>
                        ...
                        <?php if (!empty($produto->codigo_montadora)) { ?>
                            <div class="product-detail">
                                <font color="#d7d7d7"> <strong style="color: #d7d7d7">NCM: </strong><?= $produto->codigo_montadora ?></font>
                            </div>
                        <?php } ?>
                        <?php if (!empty($produto->codigo_barras)) { ?>
                            <div class="product-detail">
                                <font color="#d7d7d7"> <strong style="color: #d7d7d7">EAN: </strong><?= $produto->codigo_barras ?></font>
                            </div>
                        <?php } ?>


                        <?php if (!empty($produto->codigo_fornecedor)) { ?>
                            <div class="product-detail">
                                <font color="#d7d7d7"> <strong style="color: #d7d7d7">Codigo Fornecedor: </strong><?= $produto->codigo_fornecedor ?></font>
                            </div>
                        <?php } ?>
...
<div class="container">
    ...
    ?>
    <?php if (!empty($dadoProduto)) { ?>
        <div class="fab col-sm-1">
            <a href="" aria-disabled="true" onclick="addProdutoCarrinho('<?= $dadoProduto['pf_id'] ?>', this);return false; <?= ($ativo ?: 'disabled') ?>">
                <button type="button" class="main">
                </button>
            </a>

            <div class="col-sm-1 preco">
                <span class="valor" itemprop="lowPrice"><?= str_replace(',', ',<small>', Yii::$app->formatter->asCurrency($dadoProduto['menor_valor'])) . '</small>'; ?> </span><br>
            </div>
            <div class="col-sm-5">
