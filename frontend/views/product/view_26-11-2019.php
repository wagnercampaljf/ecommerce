...
$this->registerMetaTag(['name' => 'description', 'content' => 'Compre agora ' . $produto->nome . ' com o melhor preço no Peça Agora. Escolha o modelo de ' . $produto->subcategoria->nome . ' e aproveite nossas ofertas!']);
    ...
    $this->registerMetaTag(['itemprop' => 'name', 'content' => $produto->nome]);
    $this->registerMetaTag(['itemprop' => 'brand', 'content' =>  $produto->codigo_fabricante]);
    $this->registerMetaTag(['itemprop' => 'description', 'content' => 'Compre agora ' . $produto->nome . ' com o melhor preço no Peça Agora. Escolha o modelo de Lanternas e aproveite nossas ofertas!']);
    //$this->registerMetaTag(['itemprop' => 'description', 'content' => $produto->descricao]);
    $this->registerMetaTag(['itemprop' => 'productID', 'content' => $produto->id]);
    $this->registerMetaTag(['itemprop' => 'url', 'content' => $produto->getUrl()]);
    $this->registerMetaTag(['itemprop' => 'image', 'content' => 'https://www.pecaagora.com/site/get-link?produto_id=14874&ordem=1']);
    $this->registerMetaTag(['itemprop' => 'priceCurrency', 'content' => 'BRL']);
...
//                   echo Html::img($imagem, $options);
		...
		<?php if (!empty($produto->codigo_barras)) { ?>
                ...
                <p>
                    <button class="btn btn-link" style="background-color:white !important;"  type="button" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
